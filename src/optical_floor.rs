...
//! undecodable frames in one run. So the floor has both a per-window AND a run-wide (summed across
//! every window) term; the run-wide cap catches a wide spread of windows each individually within
//! their own per-window floor. (The run-wide cap is NOT the thing that catches a #707-class chain
//! regression — an emit-gate skip is caught by the copies/gaps tolerance + the emit-gate-skip
//! triage, and a stuck/frozen leg by frozen_leg/self-heal — never by this run-wide undecodable sum,
//! which is the test instrument's own optical read. See the issue-915 recalibration below.)
//!
...
//! temporal tear is irreducible in hardware. The floor is a data-justified value that gates LIVE.
//!
//! **Issue 915 reopened (2026-09-15): [`RUN_UNDECODABLE_FLOOR`] recalibrated 8 -> 6 -> 15.** Three
//! same-day runs on the permanent 60Hz path read run-wide undecodable 6 / 0 / 10 (one red on this
//! term alone) with a per-window max of 3; the Sep 1–4 baseline (31 runs) read 0–3, outlier 27. The
//! misses are isolated single frames ~12–15 s apart whose FAST Vernier QR was captured
//! mid-LCD-transition — the slow beat between the monitor's 60Hz refresh and the camera's 60 fps
//! shutter (pixel proofs: the fast QR blended, the right QR / grey ramps / four digital burns all
//! crisp) — exactly the irreducible optical temporal tear this floor exists for. 15 = today's
//! observed bad-phase max 10 + 50 % headroom, the same margin rule that produced the 6 (4 + 50 %).
//! The old "keep the floor below the pre-#707 regression level 10" argument NO LONGER holds: 10 is
//! now a MEASURED physical value, not a regression threshold, and a #707-class regression is caught
//! by the copies/gaps tolerance + emit-gate-skip triage / frozen_leg / self-heal — never by this
//! run-wide undecodable sum. [`PER_WINDOW_UNDECODABLE_FLOOR`] stays 4 (observed max 3).
...
///
/// KEPT at 4 by issue 905 item 3 (2026-09-04) and again by issue 915 (2026-09-15): the per-window
/// steady max is 3 across the dev1 verdicts, so 4 keeps one window of headroom. The run-wide term
/// ([`RUN_UNDECODABLE_FLOOR`], recalibrated 8 -> 6 -> 15) is the load-bearing half. No longer
/// "temporary until 120Hz" — see the module doc: the 60Hz baseline is permanent (issue 881/1179
/// closed).
...
/// load-bearing half of the floor: a per-window-only check would let a wide spread of windows —
/// each individually within its own per-window floor — through untouched.
///
/// **Recalibrated 8 -> 6 -> 15 (issue 915, 2026-09-15; was 6 per issue 905 item 3).** The 6 was
/// calibrated to a post-cam1-fix cam2-only steady max of 4. Three same-day runs after the ticket
/// reopened read run-wide undecodable 6 / 0 / 10 on the permanent 60Hz path (per-window max 3) — the
/// bad-phase max 10 is the irreducible 60Hz-vs-60fps optical temporal tear (the fast Vernier QR
/// captured mid-LCD-transition), NOT a chain regression. 15 = that bad-phase max 10 + 50% headroom,
/// the same margin rule that produced the 6 (4 + 50%). The prior "stay below the pre-#707 regression
/// level (10)" constraint is RETIRED: 10 is now a measured physical value, and a #707-class chain
/// regression is caught by the copies/gaps tolerance + emit-gate-skip triage / frozen_leg /
/// self-heal, never by this sum. See issue 915 for the full 2026-09-15 data.
pub const RUN_UNDECODABLE_FLOOR: u32 = 15;
...
/// the floor is instead re-gated to a data-justified value ([`RUN_UNDECODABLE_FLOOR`] = 15,
/// recalibrated 8 -> 6 -> 15 to the observed 60Hz bad-phase max + 50% headroom, issue 915).
/// [`window_within_floor`]/
...
mod tests {
    ...
    fn run_wide_floor_vector_ten_and_fifteen_pass_sixteen_fails_plus_per_window_boundary_915() {
        ...
        assert!(
            run_within_floor(10),
            "10 (today's bad-phase max) is within the floor 15"
        );
        assert!(
            run_within_floor(15),
            "15 is exactly the run-wide floor -> within"
        ...
        assert!(
            !run_within_floor(16),
            "16 exceeds the run-wide floor 15 -> FAIL"
        );
        assert!(
            window_within_floor(4, 846),
            "4 is exactly the per-window floor -> within"
        );
        assert!(
            !window_within_floor(5, 846),
            "5 exceeds the per-window floor 4 -> FAIL"
        );
    }
    ...
    fn gates_overall_pass_is_live_gating_again_905() {
        ...
        // floor is re-gated to a data-justified value (`RUN_UNDECODABLE_FLOOR` = 15, per issue
        // 915). The floor's
