...
/// #1355 — the N==1 RELEASE-PHASE HYSTERESIS shed decision, N==1 ONLY. Should this N==1 STEADY
/// tick shed exactly ONE EXTRA frame to step the conveyor's release phase back onto the
/// configured latency after the wall-vs-render clock drift has walked it a full frame?
///
/// ## The problem it fixes (issue 1355, measured on the stream box's `NDI 2ME PGM`)
///
/// The video release deadline keys on the WALL clock (`genlock_wall_now_ns`, DanteSync-disciplined)
/// while the render tick + audio ride the free-running monotonic/QPC clock. On the stream box the
/// two drift ~+13 ppm apart, so the boundary-locked N==1 conveyor (which advances one stamp per
/// render tick) gains one frame of on-air age every ~40 min: the A/V level saws over a ~2-frame
/// band, because the ONLY N==1 excess-shed mechanism — the #859 depth drain [`should_drain_one`] —
/// has a fixed 2-frame hysteresis ([`DRAIN_HYSTERESIS_FRAMES`]) that a 1-2 frame phase error sits
/// inside by construction. `genlock_n1_bench` reproduces this: the CURRENT rule reaches a band of
/// 2.07 frames at 13 ppm; at 0 ppm it holds 0.35.
///
/// ## Why NOT [`should_converge_phase`] (the N>=2 shed, gated off for N==1)
///
/// [`should_converge_phase`] fires at `target + quantum + budget` — a band only one SOURCE
/// interval wide — and is disabled for N==1 (`source_multiple < 2`) because at that tight
/// threshold an N==1 shed limit-cycles (a shed pulls the age just below the band; ordinary jitter
/// pushes it back within the throttle window → shed/hold/shed). This decision instead uses a
/// WHOLE-FRAME threshold: `reserve + interval − budget`. After a shed the on-air age drops a full
/// interval, landing a `budget`-width BELOW the configured hold — a whole frame below the
/// threshold — so it CANNOT re-fire until the drift re-accumulates an entire frame (~40 min).
/// That is what makes exactly one clean, jitter-immune step per crossing sustainable on N==1 where
/// the tight-band converge is not: the drift genuinely creates a one-frame surplus to shed, and
/// the whole-frame hysteresis parks the post-shed phase far from the threshold.
///
/// ## Contract
///
/// `wall_now_ns − locked_boundary_ns` is the conveyor's on-air age (the SAME comparator
/// [`should_converge_phase`] uses; at decision time `locked_boundary_ns == last_presented_ts +
/// interval`). `budget` is `max(PHASE_PIN_HYSTERESIS_NS, GENLOCK_N2_JITTER_BUDGET_NS)` — the same
/// arrival-jitter tolerance the N>=2 shed uses. The throttle counter is SHARED with the #859 drain
/// (`ticks_since_last_drain`), so at most one extra frame ever leaves the queue per tick and the
/// ≤1-drop-per-`DRAIN_MIN_TICK_INTERVAL`-ticks bound is preserved. No new per-source state.
///
/// Returns `false` for a degenerate/unknown `interval_ns` (0), an UNLOCKED boundary (0), and for
/// any `source_multiple != 1` — N>=2 stays with [`should_converge_phase`], unchanged (#1049). It
/// is a NO-OP at 0 ppm drift (the on-air age never climbs to `reserve + interval − budget`), so
/// the shipped behaviour of every settled source is byte-identical.
///
/// Mirror of the C `genlock_n1_release_phase_step()` (obs-source.c) — keep both in lock-step;
/// held identical by the `tests/genlock_relock_selection_parity.rs` executable parity gate.
pub fn n1_release_phase_step(
    wall_now_ns: u64,
    locked_boundary_ns: u64,
    latency_ms: u32,
    interval_ns: u64,
