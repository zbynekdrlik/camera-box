...
mod tests {
    ...
    /// FIX 1 round-trip (#105/#156): a SHARP dual-QR gray8 frame must survive the
    /// FULL grab-record store→decode path — the exact luma extraction the camera-box
    /// `--record-grab` mode performs ([`crate::capture::yuyv_to_gray8`]), through ffmpeg
    /// ffv1 (the dev1 listener), back through [`analyze_recording`] — and STILL decode
    /// BOTH QRs. This locks the live-rig finding (sharp grab → 100% decode) so a future
    /// regression in the gray8 plane / ffv1 params / decode pipeline fails CI, not the
    /// rig. Requires ffmpeg (the recording path's hard dependency); skipped only if the
    /// binary is genuinely absent (logged, not silently passed).
    #[test]
    fn sharp_dual_qr_gray8_round_trips_through_grab_record_store_and_decode() {
        use crate::capture::yuyv_to_gray8;
        use std::process::Command;

        if Command::new("ffmpeg").arg("-version").output().is_err() {
            // ffmpeg is a HARD dependency of the recording path; its absence is an
            // environment fault, surfaced loudly (this is not a silent skip of logic).
            panic!("ffmpeg not found — the recording-decode round-trip cannot run; install ffmpeg");
        }

        // 1) Render a SHARP dual-QR luma frame (the camera films this off cam2).
        let (w, h) = (960u32, 540u32);
        let luma = dual_qr_luma(424, 425);
        assert_eq!(luma.dimensions(), (w, h));

        // 2) Pack that luma into a YUYV buffer (luma in the even bytes, neutral 128
        //    chroma in the odd bytes) — the on-the-wire capture format — then run the
        //    EXACT grab extraction the --record-grab path uses. The extracted gray8
        //    must be byte-identical to the source luma (the path is lossless).
        ...
        // 3) Encode the gray8 plane (3 identical frames) to a lossless ffv1 mkv exactly
        //    as scripts/recording-e2e.sh's dev1 listener does (gray rawvideo → ffv1).
        ...
        // 4) Decode it back through the SAME analyze_recording the verdict uses — every
        ...
        for f in &frames {
            ...
            let ids: Vec<u32> = f.payloads.iter().map(|p| p.frame_id).collect();
            assert!(
                ids.contains(&424) && ids.contains(&425),
                "both painted ticks recovered, got {ids:?}"
            );
        }
