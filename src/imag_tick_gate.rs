...
/// #571 — the OBSERVED decimation step of a captured burn-id sequence: the most frequent forward
/// gap between consecutive present ids (the MODE of the positive deltas), floored at 1, with ties
/// resolved to the SMALLER step. This is what `burn_contiguity_in_window_with_step`'s per-emit
/// (cam1/cam3/cam4) genuine-drop check feeds to [`burn_step_contiguity`] instead of the
/// CLI-fps-derived hint — the recording's REAL 60->30 beat, which the fps args only approximate
/// and which the fixed-step assumption got wrong two ways at once (run 554307):
///
/// * a step-1 (NON-decimated) capture — every emitted id present, or a genuine single-id drop —
///   has mode 1, so a gap of 2 stays a charged REAL DROP (the #356/#208 never-mask invariant, and
///   the reason a blanket step-2 masked genuine losses in the unit captures);
/// * the Topology-v2 step-2 capture has mode 2, so a by-design gap of 2 is decimation (NOT loss)
///   and only a gap of 4+ is a real drop (the 11087 phantom cam1 "drops" the CLI-fps step-2
///   correctly ignored, but which a hardcoded step-1 charged).
///
/// The MODE (not a mean/median) is chosen so a SHORT window with one large gap cannot inflate the
/// step and silently mask the very drop that opened the gap (a 4-id window `[a, a+2, a+4, a+8]`
/// stays step 2 → the gap of 4 charges one drop; a mean would round to 3 and mask it). For a
/// genuine 2:1 decimation the delta of 2 is the plurality by construction (every ~2nd frame is
/// sampled; ±1 genlock jitter splits into a MINORITY of 1s and 3s), so the mode tracks the true
/// ratio robustly. The tie-break to the smaller delta is the safe side: a smaller step charges
/// MORE gaps, so an ambiguous window can only ever OVER-report, never MASK. Fewer than two distinct
/// ids ⇒ no delta is observable ⇒ 1 (a single/empty id sequence is trivially contiguous anyway).
pub fn observed_step(ids: &[u32]) -> u32 {
    use std::collections::{BTreeMap, BTreeSet};
    let sorted: Vec<u32> = ids
        .iter()
        .copied()
    ...
    // Tally each positive consecutive delta. `sorted` is a de-duplicated ascending run, so every
    // window delta is >= 1 and plain subtraction can never underflow.
    let mut counts: BTreeMap<u32, u32> = BTreeMap::new();
    for w in sorted.windows(2) {
        *counts.entry(w[1] - w[0]).or_insert(0) += 1;
    }
    // Mode; on an equal count the SMALLER delta wins (conservative — over-report, never mask).
    // `max_by_key` with `Reverse(delta)` as the secondary key makes the smaller delta compare
    // greater when counts tie; the deltas are distinct BTreeMap keys, so this never returns an
    // arbitrary element on a true tie.
    counts
        .into_iter()
...
mod tests {
    ...
    #[test]
    fn observed_step_of_a_clean_step2_decimation_is_2_571() {
        // The Topology-v2 cam(60)->strih(30) capture: ids step by 2 by design.
        assert_eq!(observed_step(&[2000, 2002, 2004, 2006, 2008]), 2);
        // Short window (the node-test signature) still reads step 2.
        assert_eq!(observed_step(&[2000, 2002, 2004, 2008]), 2);
    }

    #[test]
    fn observed_step_of_a_non_decimated_step1_capture_is_1_571() {
        // A full-rate capture, or a capture with a single genuine one-id gap, is mode 1 — so the
        // gap of 2 the drop opens is charged, NEVER masked (the #356/#208 never-mask invariant).
        assert_eq!(observed_step(&[5000, 5001, 5002, 5003, 5004]), 1);
        assert_eq!(observed_step(&[5000, 5001, 5002, 5004, 5005]), 1); // 5003 dropped -> gap 2
    }

    #[test]
    fn observed_step_ignores_minority_genlock_jitter_571() {
        // A 2:1 decimation with ±1 genlock jitter: deltas mostly 2, a minority of 1 and 3. The
        // mode is still 2 (2 is the plurality by construction), so jitter never flips the step.
        assert_eq!(
            observed_step(&[2000, 2002, 2003, 2005, 2007, 2009, 2010, 2012]),
            2
        );
    }

    #[test]
    fn observed_step_tie_breaks_to_the_smaller_delta_571() {
        // Equal counts of delta 1 and delta 2 ⇒ take the SMALLER (1): charges the gap-2 as a
        // candidate rather than masking it — the safe side on an ambiguous window.
        assert_eq!(observed_step(&[1000, 1001, 1003]), 1); // deltas {1,2}, one each
    }

    #[test]
    fn observed_step_of_fewer_than_two_ids_is_1_571() {
        assert_eq!(observed_step(&[]), 1);
        assert_eq!(observed_step(&[42]), 1);
        // Duplicates collapse to one distinct id ⇒ still < 2 distinct ⇒ 1.
        assert_eq!(observed_step(&[42, 42, 42]), 1);
    }

    #[test]
    fn observed_step_a_short_window_with_one_big_gap_does_not_mask_via_step_inflation_571() {
        // A mean/median would round [a, a+2, a+4, a+8] to 3 and mask the gap-4 drop; the MODE
        // stays 2, so `burn_step_contiguity` charges the one genuinely-missing slot.
        assert_eq!(observed_step(&[2000, 2002, 2004, 2008]), 2);
        assert_eq!(
            burn_step_contiguity(
                &[2000, 2002, 2004, 2008],
                observed_step(&[2000, 2002, 2004, 2008])
            )
            .missing_ids,
            vec![2006],
            "the genuine drop on top of clean step-2 decimation is charged, not masked"
        );
    }
