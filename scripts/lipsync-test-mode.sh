...
# lipsync_playback_cmds MEDIA FB_DEVICE AUDIO_DEVICE PLAYBACK_PIDFILE -- the ONE ffmpeg process
# feeding both sinks from a single demux/decode timeline (live-verified on cam2, issue 930):
# bgra pixel format (matches src/probe/fb.rs's own painter convention), -ac 2 (the ALSA device
# refused a mono stream in the live sanity test), backgrounded + its PID tracked so `stop` can
# find it. `-stream_loop -1` loops the (short, ~60s) asset continuously for an arbitrary-length
# recording window.
