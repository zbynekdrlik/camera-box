...
# ssh_ip IP CMD -- the same read-only ssh to ANOTHER fleet box (the (ao) rig-mode read off cam2).
ssh_ip() {
  sshpass -p "$CAM_PW" ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o ConnectTimeout="$SSH_TIMEOUT" \
    "${SSH_USER}@$1" "$2"
}
...
# The 24.9.2026 M.2 re-provisioning of cam1-4 came up with NO relay (no unit, no binary, no gphoto2)
# and nothing noticed: (am) grades a missing relay `na`. setup-device.sh's [bkshading-relay] sub-step
...
elif [ "$RELAY_PROV_VERDICT" = ok ]; then
  ok "bkshading relay provisioned: binary + unit (byte-matches the repo) + env + gphoto2, unit ${RELAY_EXPECTED} as rig mode '${RELAY_MODE}' (${RELAY_MODE_SRC}) wants -- issue 808"
else
  while IFS= read -r _reason; do
    [ -n "$_reason" ] && fail "bkshading relay: ${_reason#FAIL: } (rig mode '${RELAY_MODE}' from ${RELAY_MODE_SRC}, source box '${RELAY_SRC_BOX:-?}')"
  done <<< "$RELAY_PROV_VERDICT"
fi
