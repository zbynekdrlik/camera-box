...
# the match requires, INSIDE a `<context name="Root">...</context>` block, a `<mousebind
# button="Right" ...>` whose `<action name="ShowMenu">` names `<menu>root-menu</menu>`. Scoped to
# the Root mouse context ON PURPOSE -- a root-menu named only in a keybind, or a Root right-click
# bound to a DIFFERENT menu, must NOT pass. Tolerant of both XML attribute quote styles
# ([\x22\x27] = " or '). Assert-only (#1095 design (b)): the acceptance gate fails loud, it never
# rewrites a hand-tuned operator rc.xml. Runs LOCALLY on dev1 against the ssh-read text (GNU grep
# 3.11 and ugrep both provide -P/PCRE2), never on the box.
imag_openbox_root_menu_bound() {
  ...
  printf '%s' "$root_block" | grep -qP '<mousebind\s+button=[\x22\x27]Right[\x22\x27][^>]*>.*?<action\s+name=[\x22\x27]ShowMenu[\x22\x27]\s*>\s*<menu>\s*root-menu\s*</menu>'
}
