...
latency_drift_check() {
  ...
  if [ "${alert_now:-0}" = "1" ]; then
    ...
    python3 "$NOTIFY" notify --body \
      ...
      --dedup-key "$(watchdog_notify_key "imag-obs-latency-drift" "$(date +%s)")" \
...
restart_storm_check() {
  ...
  if [ "${alert_now:-0}" = "1" ]; then
    ...
    python3 "$NOTIFY" notify --body \
      ...
      --dedup-key "$(watchdog_notify_key "imag-obs-restart-storm" "$(date +%s)")" \
...
main() {
  ...
  if [ "${alert_now:-0}" = "1" ]; then
    ...
    python3 "$NOTIFY" notify --body \
      ...
      --dedup-key "$(watchdog_notify_key "imag-obs-down" "$(date +%s)")" \
