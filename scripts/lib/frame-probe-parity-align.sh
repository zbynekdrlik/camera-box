...
# stays on the last-main build. recording-e2e.sh's [1/8] pin then flagged a lag every run but
# nothing redeployed it -- exactly the live 2026-08-29 incident (deployed 510da513aac7 LAGGED, hand-
...
# A failed/partial align just leaves the [1/8] HARD pin (issue 1235) to REFUSE the run, naming the fix.
...
#   NOCANDIDATE  candidate sha empty                  -> no align; the [1/8] hard pin REFUSES
...
frame_probe_align_resolve_ci_bin() {
  ...
  if [ -n "${FRAME_PROBE_ALIGN_ARTIFACT_DIR:-}" ]; then
    ...
    command -v gh >/dev/null 2>&1 || {
      echo "[0/8] frame-probe parity auto-align (#1138): gh CLI unavailable -- cannot source the candidate frame-probe artifact; the [1/8] hard pin below REFUSES the run (fail-closed, issue 1235)." >&2
      printf ''
      return 0
    }
    ...
    if [ -z "$sha" ]; then
      echo "[0/8] frame-probe parity auto-align (#1138): no candidate commit sha resolvable (FRAME_PROBE_ALIGN_CANDIDATE_SHA / GITHUB_SHA / git rev-parse HEAD all empty) -- cannot source probe-tools-linux-amd64; the [1/8] hard pin below REFUSES the run (fail-closed, issue 1235)." >&2
      printf ''
      return 0
    fi
    local run_id
    ...
    if [ -z "$run_id" ]; then
      echo "[0/8] frame-probe parity auto-align (#1138): no successful ci.yml run for commit ${sha:0:12} to source probe-tools-linux-amd64 (self-heals once ci.yml publishes it, issue 1245); the [1/8] hard pin below REFUSES the run (fail-closed, issue 1235)." >&2
      printf ''
      return 0
    fi
    ...
    if ! gh run download "$run_id" --repo "$repo" -n probe-tools-linux-amd64 --dir "$dir" >/dev/null 2>&1; then
      echo "[0/8] frame-probe parity auto-align (#1138): could not download probe-tools-linux-amd64 from ci.yml run $run_id; the [1/8] hard pin below REFUSES the run (fail-closed, issue 1235)." >&2
      printf ''
      return 0
    fi
  fi
  fp="$dir/frame-probe"
  if [ ! -f "$fp" ]; then
    echo "[0/8] frame-probe parity auto-align (#1138): candidate frame-probe missing in artifact ('$fp'); the [1/8] hard pin below REFUSES the run (fail-closed, issue 1235)." >&2
    printf ''
    return 0
  fi
  ...
  if [ "${FRAME_PROBE_ALIGN_SKIP_VERSION_GUARD:-0}" != "1" ]; then
    ...
    if [ ! -x "$cbp" ]; then
      echo "[0/8] frame-probe parity auto-align (#1138): probe-tools artifact has no camera-box-probe to version-guard against; the [1/8] hard pin below REFUSES the run (fail-closed, issue 1235)." >&2
      printf ''
      return 0
    fi
...
# local build. ALWAYS returns 0 (the [1/8] hard pin is the loud signal). SKIPPED under --no-main-pin.
frame_probe_parity_align_before_gate() {
  ...
  if [ -z "$ci_bin" ]; then
    echo "[0/8] frame-probe parity auto-align (#1138): no candidate frame-probe resolved -- the [1/8] hard pin below REFUSES the run (fail-closed, issue 1235)."
    return 0
  fi
  ...
  if [ "${#entries[@]}" -eq 0 ]; then
    echo "[0/8] frame-probe parity auto-align (#1138): no painter node in the list -- nothing to align; the [1/8] hard pin below REFUSES the run (fail-closed, issue 1235)."
    return 0
  fi
  local action
  ...
  case "$action" in
    ALIGN)
      ...
      if frame_probe_align_deploy "$ci_bin" "${names[*]}"; then
        echo "[0/8] frame-probe parity auto-align (#1138): deploy complete -- cam2's painter is now on the candidate build; the [1/8] hard pin below confirms it (issue 1235)."
      else
        echo "[0/8] frame-probe parity auto-align (#1138): WARNING -- deploy did NOT complete cleanly; the [1/8] hard pin below REFUSES the run + names the manual fix (deploy-fleet.sh --frame-probe, issue 1235)." >&2
      fi
      ;;
    OK)
    ...
    NOACTIVE)
      ...
      ;;
    UNKNOWN)
      echo "[0/8] frame-probe parity auto-align (#1138): painter frame-probe sha unread -- no auto-align; the [1/8] hard pin below REFUSES the run (hard gate, issue 1235)." >&2
      ;;
    NOCANDIDATE)
      echo "[0/8] frame-probe parity auto-align (#1138): no candidate sha resolvable -- skipping align; the [1/8] hard pin below REFUSES the run (fail-closed, issue 1235)." >&2
      ;;
  esac
  return 0
}
