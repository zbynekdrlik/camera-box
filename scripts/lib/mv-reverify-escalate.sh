...
mv_reverify_probe_raw() {
  ...
  if [ -n "${MV_REVERIFY_RECEIVED_CMD:-}" ]; then
    ...
    # callers (the frozen-input watchdog + the Tier-0 harness).
    local _ps _enc
    _ps="gc (gci \$env:APPDATA\\obs-studio\\logs\\*.txt | sort LastWriteTime | select -last 1).FullName -Tail ${MV_REVERIFY_RECEIVED_TAIL:-400}"
    _enc="$(printf '%s' "$_ps" | iconv -f UTF-8 -t UTF-16LE | base64 -w0)"
