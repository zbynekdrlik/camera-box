...
# over a bounded retry window before giving up with an HONEST one-line fault. issue 1317: the restart
# is class-resolved via the shared bundle_state_restart_remote_cmd -- the Linux strih-lx (a systemd
# --user unit) gets `systemctl --user restart`, never the Windows schtasks line.
#
...
#   issue 1317: an optional 3rd arg <class> (the obs-fleet class) makes the Recover hint
#   platform-true -- a `linux-genlock` box (strih-lx) names its systemd --user unit; anything else
#   (the no-arg default) keeps the byte-identical Windows hint.
...
#   a session-agnostic restart + a bounded retry, /bundle-state.json was (re)fetched to <dest>.
#   issue 1317: [class] is the box's obs-fleet class; the restart command is resolved from it by the
#   shared bundle_state_restart_remote_cmd (linux-genlock -> systemctl --user, else schtasks), so a
#   Linux box is never sent the Windows restart. Omitted = the pre-1317 Windows behaviour.
...
bundle_state_selfheal_fetch() {
  ...
  # Reuse the issue-732 lib's exact, safety-pinned restart command + its label (ONE source of truth
  # -- never re-invent the restart string here). Lazy-source so this stays a pure source-only lib.
  command -v bundle_state_restart_remote_cmd >/dev/null 2>&1 \
    || . "${BASH_SOURCE[0]%/*}/bundle-state-health.sh"
  local how
  how="$(bundle_state_restart_label "$class")"
  echo "    bundle-state-server on ${host}:${port} not answering -- issuing session-agnostic restart ($how, #817)" >&2
  if command -v sshpass >/dev/null 2>&1; then
    timeout "$ssh_timeout" sshpass -p "$ssh_pw" ssh -o StrictHostKeyChecking=no -o ConnectTimeout=10 \
      "${ssh_user}@${host}" "$(bundle_state_restart_remote_cmd "$class")" >/dev/null 2>&1 \
      || echo "    NOTE: the $how restart call to ${host} did not return cleanly (ssh/creds/no unit?) -- still polling :${port}" >&2
  else
  ...
  bundle_state_down_message "$host" "$port" "$class" >&2
  return 1
}
