...
# --- CLOCK DISCIPLINE (issue 1372, dantesync 1.9.0 / dantesync#117) ---------------------------
#
# dantesync 1.9.0 made `ptp_phase_lock` the default discipline: the rate AND phase come from the
# Dante PTP tick only, NTP only moves the DATE, and phase slew is not used (`phase_slew_enabled`
# reads false BY DESIGN). `clock_discipline="legacy"` in the config restores the pre-1.9.0 servo,
# where phase_slew is again the cure for the NTP step storm (#1215/#1130). So the bare phase_slew
# flag no longer tells a healthy node from a broken one; the node's DISCIPLINE does. This is the
# ONE classifier every consumer calls (dantesync-gate.sh, verify-imag.sh, the maintenance gate);
# its python twin is dantesync_fleet.classify_clock_discipline, and both are pinned by ONE table
# (tests/fixtures/dantesync_clock_discipline_1372.tsv) whose @ rows are live-captured /status.

# clock_discipline_from_pipe_json TEXT -> the `"clock_discipline"` string value ("" if absent,
# null, empty or unparseable -- an older build's blob has no such field). `|| true` survives a
# no-match under set -e/pipefail (the shared *_from_pipe_json convention).
clock_discipline_from_pipe_json() {
  printf '%s' "$1" | grep -oE '"clock_discipline"[[:space:]]*:[[:space:]]*"[^"]*"' \
    | sed -n 's/.*"clock_discipline"[[:space:]]*:[[:space:]]*"\([^"]*\)".*/\1/p' | tail -1 || true
}
...
# `ptp_phase_lock` with ptp_phase_locked=false is UNKNOWN here; clock_discipline_check names it
# PTP-PHASE UNLOCKED and fails it (rc 2), so it never reads as a merely incomplete status.
...
#   ptp_phase_lock + ptp_phase_locked=false -> 2, named PTP-PHASE UNLOCKED
...
clock_discipline_check() {
  ...
  if [ "$(clock_discipline_from_pipe_json "$text")" = ptp_phase_lock ] \
     && [ "$(ptp_phase_locked_from_pipe_json "$text")" = false ]; then
...
# --- DATE MASTER (issue 1372, dantesync 1.9.0 / dantesync#88) ---------------------------------
#
# Under ptp_phase_lock the NTP master (`date_authority=master`) owns the fleet DATE: it lets the
# fleet line sit up to `date_step_bound_ms` (default 50) off UTC and then announces a COORDINATED
# fleet step. Its own ntp_offset_us is that fleet-line error, so grading it on the 2 ms UTC bound
# fails a healthy master (-25 ms is normal). The master is graded on
#   |date_offset_error_ms| <= date_step_bound_ms + margin
# instead. A non-master (follower / local / legacy "") is not graded here at all.

# _pipe_json_number_raw TEXT KEY -> the raw text of a numeric-or-null JSON value for KEY ("" when
# absent). Numbers may carry a fraction/exponent (the date fields are f64 milliseconds).
...
# the same as the python twin's round()); "" for null/absent/non-numeric.
_ms_to_us() {
  case "$1" in
    ""|null) printf '' ;;
    *) awk -v v="$1" 'BEGIN { printf "%.0f", v * 1000 }' ;;
  esac
}
...
#   unknown -- master, but the error or a positive step bound is unreadable (never an OK)
date_master_verdict() {
...
# a date master with a positive numeric step bound, else BOUND_US unchanged (every other node).
...
date_master_check() {
  ...
  case "$(date_master_verdict "$text" "$margin")" in
    ...
    out)
      ...
      return 2 ;;
    *)
      printf '  %-14s DATE MASTER UNKNOWN (date_offset_error_ms=%s date_step_bound_ms=%s unreadable -- status incomplete, #1372)\n' \
        "$label" "${err:-<absent>}" "${bound:-<absent>}"
      return 3 ;;
  esac
}
...
# verify-imag fallback). When the freshest line is the date master's `(date authority, ...)` shape,
# the node is graded MEDIAN-ONLY on its own step bound + DATE_MARGIN_US -- the fleet line walks by
# design and a coordinated step moves every sample at once, so the spread is not a health signal
# (the dantesync-gate.sh master is median-only for the same reason, #1014). Every other journal is
# graded exactly as before (BOUND_US + STABILITY_US).
dantesync_journal_clock_verdict() {
  local journal="$1" fresh="$2" bound="$3" stability="$4" margin="$5" step
  step="$(date_step_bound_us_from_journal "$journal")"
  if [ -n "$step" ] && grep -qE '^[0-9]+$' <<<"$margin" && [ "$step" -gt 0 ]; then
    dantesync_offset_verdict "$journal" "$fresh" "$((10#$step + 10#$margin))"
  else
