...
EOF
    ...
    # #295/#307: fail-closed — never pin a grub default that cannot boot. Validate the generated
    # default menuentry references BOTH a kernel image AND an initrd before pinning it, mirroring
    # setup.sh/setup-device.sh. The single image kernel got its initrd from configure_overlay's
    # `update-initramfs -u` (run before this step), but assert it rather than assume it — a default
    # entry without an initrd is exactly the brick that took down CAM3/CAM4.
    local grub_cfg="${WORK_DIR}/rootfs/boot/grub/grub.cfg"
    [[ -f "$grub_cfg" ]] || error "#307: $grub_cfg was not generated by update-grub — cannot validate the grub default"
    local default_entry
    ...
    if ! grep -qE '(vmlinuz|[[:space:]]linux )' <<<"$default_entry" \
        || ! grep -q 'initrd' <<<"$default_entry"; then
        error "#307: grub default entry lacks a kernel image or initrd line — refusing to build a brickable image"
    fi
