...
STATE="${IMAG_POWER_GUARD_STATE:-$IMAG_POWER_GUARD_STATE_FILE}"
...
if {
  ...
  printf 'COOL=%s\n' "$NEW_COOL"
  printf 'STEPPED=%s\n' "$NEW_STEPPED"
  # Record the step-down watts THIS guard uses so verify-imag.sh compares the observed clamp value
  # against the guard's OWN authority, not an independent env default that could diverge from a
  # provisioning-time IMAG_PL1_STEPDOWN_W override (#1188). Distinct key from the guard's own
  # STEPDOWN_W/STEPDOWN_UW vars so sourcing this file back never clobbers them.
  printf 'GUARD_STEPDOWN_W=%s\n' "$STEPDOWN_W"
} > "$_tmp" 2>/dev/null; then
  # World-readable: verify-imag.sh consults STEPPED + GUARD_STEPDOWN_W over a NON-root SSH (newlevel)
  # to tell a legitimate thermal step-down from foreign drift (#1188). The file holds only
  # HOT/COOL/STEPPED + the step-down watts -- no secret. chmod the tmp BEFORE the atomic mv so the
  # installed file is already 0644.
