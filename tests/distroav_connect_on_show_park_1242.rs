...
#[test]
fn a_park_blanks_the_source_so_a_show_never_presents_a_stale_frame() {
    // KEEP_CONTENT is certified, so without an explicit blank the last pre-park frame (possibly hours
    // old) is presented as if live on show; obs_source_output_video(NULL) also resets the genlock FIFO
    // (last_frame_ts = 0), so a parked input stops counting an underrun every render tick.
    let src = squish(&read(NDI_SOURCE));
    let park_at = src
        .find("ndi_reap_receiver_detached(ndiLib, park_frame_sync, park_receiver);")
        .expect("issue 1242: the park reap call is gone");
    let unpark_at = src
        .find("state=unparked")
        .expect("issue 1242: the unpark line is gone");
    assert!(
        src[park_at..unpark_at].contains("deactivate_source_output_video_texture(s);"),
        "{NDI_SOURCE}: issue 1242 — the park must blank the source \
         (deactivate_source_output_video_texture) before it idles."
    );
}
