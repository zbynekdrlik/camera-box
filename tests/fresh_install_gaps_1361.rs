...
//!
//! - G2: ONE shared RemoteOS MCP installer, `scripts/lib/remoteos-mcp.sh`, used by setup-strih step
//!   10, setup-imag step 23 and setup-device STEP 17b: the project's GitHub API tarball (the GH token
//!   on curl's STDIN, never argv) pip-installed into `/opt/remoteos-mcp-venv`, the live strih-lx unit
//!   shape with the key in a 0600 EnvironmentFile, idempotent, graded by verify-strih item 8.
...
//! `tests/obs_box_baseline_1357.rs` next to the other baseline rows.
//!
//! Tier-0: sourced bash + PATH stubs (curl / apt-get / systemctl / dpkg-deb) + a fake venv, no
//! root, no network, no rig.
