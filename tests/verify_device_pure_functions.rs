...
#[test]
fn no_publish_30p_check_remains_1342() {
    let body = std::fs::read_to_string(script()).unwrap();
    for needle in ["publish_30p", "PUBLISH_30P", "publish-30p", "(30p)"] {
        assert!(
            !body.contains(needle),
            "verify-device.sh must not grade the removed 30p stream any more ({needle}, issue 1342)"
        );
    }
}

#[test]
fn check_an_ndi_discovery_is_wired_into_the_live_flow_1342() {
    let body = std::fs::read_to_string(script()).unwrap();
    let guard_pos = body
    ...
    let an = live_flow
        ...
        .expect("the (an) exec block must exist");
    let q = live_flow
        .rfind("# (q) .bak cruft drift")
        .expect("the (q) block must exist");
    assert!(
        an < q,
        "(an) must sit before (q), which stays the LAST check"
    );
}
