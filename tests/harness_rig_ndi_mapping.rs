...
fn active_map_defaults_to_exactly_the_two_active_cameras() {
    // #827/#898/issue 947: with no override, active_map() (and therefore parse_map_args(None),
    // the no-`--map` path main() takes) must resolve to exactly cam1/cam2 -- the
    // DEFAULT_ACTIVE_SET fallback (cam3 retired 2026-07-31, grabber card destroyed; cam4 retired
    // 2026-08-02, grabber wedges the capture leg within minutes of every start).
    let (stdout, stderr) = run_py_check(
        r#"import os
os.environ.pop("CAMERA_ACTIVE_SET", None)
want = m.active_map()
senders = sorted(s.split(" ", 1)[0] for _, s in want)
assert senders == ["CAM1", "CAM2"], senders
assert m.parse_map_args(None) == want, "no --map -> active_map()"
"#,
    );
    assert!(
        stdout.contains("OK"),
        "issue 947 active_map default checks failed:\nstdout:{stdout}\nstderr:{stderr}"
    );
}
...
fn default_active_set_env_var_matches_camera_set_sh_exactly() {
    ...
    assert!(
        py.contains(r#"os.environ.get("CAMERA_ACTIVE_SET", "cam1 cam2")"#),
    ...
    assert!(
        sh.contains(r#"CAMERA_ACTIVE_SET="${CAMERA_ACTIVE_SET:-cam1 cam2}""#),
