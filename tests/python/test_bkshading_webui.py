#!/usr/bin/env python3
"""M1 static verification of the bkshading web panel (issue 808).
...
(preview on top, params below), the version-on-dashboard label, the shading controls, and
that the JS talks to the real service API and carries no localhost URLs. Runnable directly
...
def test_index_has_versioned_block_skeleton():
    ...
    assert 'data-role="preview"' in html, "each block has a preview area (M1 placeholder)"
    assert "NDI preview" in html, "M1 preview is a placeholder"
