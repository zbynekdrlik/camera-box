...
/// #1355 — the SUSTAINED-EXCESS one-frame drain for N==1 must be present and WIRED. The +13 ppm
/// wall-vs-QPC clock drift gains one frame every ~40 min on the deep N==1 NDI 2ME PGM FIFO; the
/// #859 depth drain (target + 2 hysteresis) and the N>=2-only #1049 converge both structurally
/// miss it, so a persistent one-frame excess saws the hold over two frames. Mirror of
/// src/genlock_backlog.rs should_sustain_drain (Tier-0 tested) + the C-vs-Rust parity gate
/// tests/genlock_relock_selection_parity.rs.
#[test]
fn sustain_drain_present_and_wired_in_1355() {
    let internal = squish(&vendor_file(OBS_INTERNAL));
    assert!(
        internal.contains("uint32_t genlock_excess_run_ticks;"),
        "{OBS_INTERNAL}: #1355 — the sustained-excess run counter genlock_excess_run_ticks is \
         gone; the drift-vs-jitter discriminator has no state to accumulate."
    );
    assert!(
        internal.contains("uint32_t genlock_sustain_sheds;"),
        "{OBS_INTERNAL}: #1355 — the distinct sustain-shed observability counter \
         genlock_sustain_sheds is gone."
    );

    let src = squish(&vendor_file(OBS_SOURCE));
    assert!(
        src.contains("#define GENLOCK_DRAIN_SUSTAIN_TICKS 300"),
        "{OBS_SOURCE}: #1355 — the sustain-window constant (GENLOCK_DRAIN_SUSTAIN_TICKS, 10 s at \
         30 fps) is missing or changed; a shorter window would shed on arrival jitter. Mirror: \
         src/genlock_backlog.rs DRAIN_SUSTAIN_TICKS."
    );
    assert!(
        src.contains("#define GENLOCK_DRAIN_SUSTAIN_HYSTERESIS_FRAMES 1"),
        "{OBS_SOURCE}: #1355 — the sustain hysteresis constant (must stay 1) is missing or \
         changed; > 1 would let the hold re-widen to the #859 2-frame band. Mirror: \
         src/genlock_backlog.rs DRAIN_SUSTAIN_HYSTERESIS_FRAMES."
    );
    // The PURE liftable decision (the parity gate compiles this standalone).
    assert!(
        src.contains("static inline bool genlock_sustain_drain_due("),
        "{OBS_SOURCE}: #1355 — the pure sustained-excess decision genlock_sustain_drain_due is \
         gone; the drift frame would never be shed. Mirror: src/genlock_backlog.rs \
         should_sustain_drain + the C-vs-Rust parity gate."
    );
    // Wired IN, on the drain_eligible (N==1 steady) path, with the per-source run + shared throttle.
    assert!(
        src.contains(
            "genlock_sustain_drain_due(sustain_depth, sustain_target, source->genlock_excess_run_ticks,"
        ),
        "{OBS_SOURCE}: #1355 — genlock_sustain_drain_due is defined but no longer CALLED with the \
         per-source excess-run + shared-throttle counters from the N==1 steady release tail; the \
         drain would be dead code and the FIFO would keep sawing over two frames."
    );
    // A sustain shed increments a DISTINCT counter for the audit line (log silence lies).
    assert!(
        src.contains("source->genlock_sustain_sheds++;"),
        "{OBS_SOURCE}: #1355 — the distinct sustain-shed observability counter increment is gone."
    );
    assert!(
        src.contains("sustain_sheds=%u"),
    ...
    assert!(
        raw_sustain_erases_index_zero(&vendor_file(OBS_SOURCE)),
        "{OBS_SOURCE}: #1355 — the sustain shed must erase array[0] (the would-be-presented frame) \
         and present the next; dropping the one BEHIND it is the self-cancelling no-op the drain \
         call-site comment documents."
    );
}

/// Structural check: within the #1355 sustain shed block, the FIRST `da_erase` erases index 0.
fn raw_sustain_erases_index_zero(raw: &str) -> bool {
