...
fn wedge_verdict_frozen_counter_is_a_wedge() {
    ...
    assert_eq!(v, "WEDGE", "#1093: received= did not advance (curr==prev) -> receiver wedge");
}

#[test]
fn wedge_verdict_advancing_counter_is_not_a_wedge() {
    let (_ok, v) = run("mv_reverify_wedge_verdict 100 160");
    assert_eq!(v, "NO_WEDGE", "#1093: received= advanced -> frames flowing, not a wedge");
}
...
fn wedge_verdict_counter_reset_is_not_a_wedge() {
    ...
    assert_eq!(v, "NO_WEDGE", "#1093: a counter reset (curr<prev) is recv resuming, never a false wedge");
}
...
fn wedge_verdict_absent_recv_line_is_a_wedge() {
    ...
    assert_eq!(v, "WEDGE", "#1093: no recv line at all (empty curr) is the 'no recv' wedge");
}
...
fn wedge_verdict_first_reading_with_no_prior_is_not_a_wedge() {
    ...
    let (_ok, v) = run("mv_reverify_wedge_verdict '' 100");
    assert_eq!(v, "NO_WEDGE", "#1093: a first numeric reading with no prior is not a wedge");
}
...
fn painter_up_cmds_reuse_the_presenter_aware_painting_signal() {
    ...
    assert!(text.contains("cam2-painter.service"),
...
fn run_painter_up(active: &str, journal: &str, fuser_ok: bool, iters: &str) -> (i32, String) {
    ...
    write_fake("systemctl", &format!("#!/usr/bin/env bash\nprintf '%s' '{active}'\n"));
    // journalctl -u ... prints the fixed journal text.
    write_fake("journalctl", &format!("#!/usr/bin/env bash\ncat <<'J'\n{journal}\nJ\n"));
    write_fake(
        "fuser",
    ...
    let gen = format!(
        "set -uo pipefail\n. \"{}\" 2>/dev/null\nMV_REVERIFY_PAINTER_UP_ITERS={} mv_reverify_painter_up_cmds",
        lib_path().display(),
        iters
    );
    let cmds = Command::new("bash").arg("-c").arg(&gen).output().expect("gen");
...
fn painter_up_cmds_confirm_a_genuinely_painting_kms_painter() {
    ...
    assert_eq!(code, 0, "#1093(a): a painting KMS painter must exit 0; got out={out}");
    assert!(out.contains("PAINTER_UP"), "#1093(a): expected PAINTER_UP, got {out}");
}
...
fn painter_up_cmds_report_not_confirmed_when_the_painter_is_dark() {
    ...
    assert_ne!(code, 0, "#1093(a): a dark painter must exit non-zero; got out={out}");
    assert!(out.contains("PAINTER_NOT_CONFIRMED"), "#1093(a): expected PAINTER_NOT_CONFIRMED, got {out}");
}
...
fn obs_restart_ps_is_headless_safe_kill_plus_sentinel_only() {
    ...
    assert!(ps.contains("Stop-Process") && ps.contains("obs64"),
    ...
    assert!(!ps.to_lowercase().contains("stop-process -name autohotkey")
...
fn probe_received_extracts_the_newest_counter_for_the_named_source() {
    ...
    assert_eq!(v, "740", "#1093(b): must extract the NEWEST received= for the named source (not cam2's)");
}
...
fn probe_received_is_empty_when_the_source_has_no_audit_line() {
    ...
    let _ = fs::remove_file(&stub);
    assert_eq!(v, "", "#1093(b): an absent audit line -> empty reading (drives the 'no recv' wedge)");
}
...
fn run_orchestrator(preflight_body: &str, received_cmd: &str) -> (String, i32) {
    ...
    let out = Command::new("bash").arg("-c").arg(&script).output().expect("orchestrator");
...
fn orchestrator_passes_straight_through_when_the_leg_is_live() {
    ...
    assert_eq!(rc, 0, "#1093: a live leg returns 0 with no escalation; out=\n{out}");
    assert!(!out.contains("RESTARTED"), "#1093: no OBS restart when the leg is live; out=\n{out}");
}
...
fn orchestrator_restarts_obs_once_and_recovers_on_a_receiver_wedge() {
    ...
    let _ = fs::remove_file(&cnt);
    assert!(out.contains("RESTARTED"), "#1093(b): a receiver wedge must restart strih OBS; out=\n{out}");
    assert_eq!(rc, 0, "#1093(b): the leg must recover after the restart + re-check; out=\n{out}");
}
...
fn orchestrator_does_not_restart_obs_when_recv_is_advancing() {
    ...
    let (out, rc) = run_orchestrator("preflight_mv_reverify() { return 1; }", &advancing_rx_stub());
    assert!(!out.contains("RESTARTED"),
        "#1093(b): an advancing received= is a dead SOURCE, never a receiver wedge -- no OBS restart; out=\n{out}");
    assert_eq!(rc, 1, "#1093(b): a genuinely dead leg fails loud (rc=1); out=\n{out}");
}
...
fn orchestrator_restarts_obs_at_most_once_per_run() {
    ...
    assert!(extract_rc(&combined) == 1, "#1093(b): a still-wedged leg after the one restart fails loud; out=\n{combined}");
}
...
fn recording_e2e_sources_the_escalate_lib() {
    ...
    assert!(s.contains("lib/mv-reverify-escalate.sh"),
...
fn recording_e2e_deploy_sites_use_the_guarded_reverify() {
    ...
    assert!(s.contains(r#"mv_reverify_or_escalate "$_cn" "${_cn#cam}""#),
    ...
    // own use stay.
    assert!(!s.contains(r#"preflight_mv_reverify "$CAMERA_NAME" "${CAMERA_NAME#cam}" || exit 1"#),
...
fn painter_up_wait_runs_once_before_the_cam1_probe() {
    ...
    assert_eq!(waits.len(), 1,
        "#1093(a): the painter-up wait fires exactly ONCE (before cam1's probe) -- NOT before the \
         ALL_CAMBOX loop, where the painter is deliberately stopped; found {}", waits.len());
    let wait_at = waits[0].0;
    let cam1_call = s.find(r#"mv_reverify_or_escalate "$CAMERA_NAME""#)
        .expect("#1093: cam1 guarded reverify call must exist");
    assert!(wait_at < cam1_call,
...
fn cleanup_reverify_stays_warn_only_and_never_escalates() {
    ...
    let s = recording_e2e_text();
    let start = s.find("cleanup_mv_reverify_active_boxes() {")
        .expect("cleanup wrapper must exist");
    let end = s[start..].find("\n}\n").map(|i| start + i).expect("wrapper closes");
