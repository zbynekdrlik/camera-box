	...
	while (s->running) {
		...
		// break is a permanent reattach-proof death); ndi_source_name is never touched.
		{
			...
			if (parked_1242) {
				...
				// a fresh issue-767 stale window, and a fresh #1096 no-connection timer. The #1180
				// post-connect identity verify is KEPT for this bind: after a long park the sender may
				// have restarted and reshuffled its port (the wrong-camera guard outweighs its ~1 s
				// one-shot, which is part of the accepted cold start).
