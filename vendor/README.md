...
| `ndi_input_latency` | `2` | DistroAV NDI **input** `latency` mode = **Lowest** (the obs-websocket `GetInputSettings` `latency` field; `2`=Lowest, `1`=Low, `0`=Normal). This is the zero-loss-certified ingest mode — it matches `scripts/obs_phase2.py` `_PROBE_NDI_SETTINGS` (`latency: 2`), the harness that certified the path. It is checked on the **genlocked broadcast-path inputs**: on strih the camera ingests (`NDI cam5`=CAM1, `NDI cam1`=CAM3, `NDI cam3`=CAM4), on stream the strih→stream program feed (`NDI 2ME PGM`). Re-pin only on a deliberate latency rollout; an input drifted off `2` (seen 2026-06-16: strih `NDI cam5` and stream `NDI 2ME PGM` had drifted to `0`, adding ingest buffering latency — #84) is drift the guard flags. Non-broadcast inputs (preview/CG/lyrics) are out of scope of the pin |
