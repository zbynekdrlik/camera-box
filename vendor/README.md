...
| `genlock_build_sha_imag` | *(no static pin — #531 dynamic check)* | `/opt/obs-genlock/GENLOCK_BUILD_SHA.txt` on imag-nb (the commit SHA `scripts/setup-imag.sh` writes on every hot-swap, #460). **#531: NO static pin any more** — the pre-#531 empty-pin compare was inert (always UNKNOWN, could never FAIL, so it never caught a merged-but-never-deployed genlock change — the #530 45fps disaster). `--check-imag` compares this box SHA **DYNAMICALLY** against origin/main's vendored-genlock HEAD, using a `git merge-base`-scoped model (**#1292**, replacing the original plain-ancestry `git log <box>..origin/main` range — that range read a box on the dev candidate line as FALSELY STALE the moment its own dev lineage diverged from a merge commit's DAG shape, even when the box already carried that merge's vendor content: this repo's two-branch workflow never merges main's own merge commits back into dev, so a merge commit is never a git-ancestor of a later dev commit even though its content already IS): `git log $(git merge-base <box> origin/main)..origin/main -- vendor/obs-studio vendor/distroav` non-empty = the box is genuinely missing vendor content main has = **STALE = DRIFT** (fail loud); empty = the box has nothing missing, further split by `git log origin/main..<box> -- vendor/…`: also empty = **OK (current)**; non-empty AND `git merge-base --is-ancestor <box> origin/dev` = **OK (N vendored-genlock commit(s) AHEAD of origin/main on the dev candidate line)** — a recognized release-candidate build; non-empty and NOT reachable from origin/dev either = **ORPHAN = DRIFT** (early-gate-pin doctrine: an unrecognized/orphan release must SCREAM, never pass quietly). So this row is no longer read by the guard — the authoritative "what should be deployed" is origin/main (+ origin/dev for a recognized candidate), not a hand-maintained pin |
