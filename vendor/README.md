...
|---|---|---|
| `output_fps_strih` | `60` | OBS log `video settings reset: … fps: <n>/1` on STRIH (10.77.9.202) — the LED-wall IMAG program renders 60fps (cam1 emits 60fps NDI; strih renders 60; full-chain zero-loss proven at 60). A drop back to `30` is drift. #11 mixed 60/30 topology: cam(60)→strih(60)→stream(30) |
| `output_fps_stream` | `30` | OBS log `video settings reset: … fps: <n>/1` on STREAM (10.77.9.204) — the stream box DECIMATES the 60fps strih feed to 30fps output (every other frame, via the genlock FIFO) for the restreamer. A drift UP to `60` (or any other value) is drift. #11 mixed 60/30 topology |
...
| `genlock_source_latency_stream` | `NDI 2ME PGM=450` | OBS log `genlock-fifo audit 'NDI 2ME PGM': … latency_ms=450 …` on STREAM (#357). The `NDI 2ME PGM` source (the strih→stream program feed) carries a **deliberate per-source A/V-align override**: 450 ms slows the video path to sync with the ~1 s-late mastered audio (the broadcast A/V-sync design — NOT drift; see memory + genlock.rs:598). Pin value `450` is intentional; a drift to any other value (e.g. 900 ms — the accidental double-override failure mode) FAILS the gate. Re-pin here only on a deliberate A/V-align rollout. Read-off: win-stream-snv MCP `EventLog` filtered for `genlock-fifo audit` |
