...
void OBSBasic::PasteShowHideTransition(obs_sceneitem_t *item, bool show, obs_source_t *tr, int duration)
{
	...
	std::string undo_data = OBSDataGetJsonSafe(oldTransitionData, "OBSBasic_Transitions paste-show-hide undo");
	std::string redo_data = OBSDataGetJsonSafe(transitionData, "OBSBasic_Transitions paste-show-hide redo");
...
QMenu *OBSBasic::CreateVisibilityTransitionMenu(bool visible)
{
	...
	auto setTransition = [this](QAction *action, bool visible) {
		...
		std::string undo_data = OBSDataGetJsonSafe(oldTransitionData, "OBSBasic_Transitions visibility-menu undo");
		std::string redo_data = OBSDataGetJsonSafe(newTransitionData, "OBSBasic_Transitions visibility-menu redo");
		if (undo_data.compare(redo_data) != 0) {
