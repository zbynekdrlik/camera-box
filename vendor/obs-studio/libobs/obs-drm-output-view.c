 ...
 * MULTIVIEW tick this TU claims a scanout buffer through the shared mailbox seam
 * (obs-drm-output-internal.h), binds it as the render target at the connector mode size and calls
 * the renderer into it — one render, no extra full-frame copy.
 *
 ...
 * atomic temp-file rename). Caller holds persist_lock. */
static bool drm_output_view_persist_locked(const char *name)
{
	...
	if (json) {
		...
		dstr_cat(&line, "\n");
		ok = os_quick_write_utf8_file_safe(g_view.config_path, line.array, line.len, false, "tmp", NULL);
		dstr_free(&line);
	}
...
bool obs_drm_output_set_view(enum obs_drm_output_view view)
{
	...
	pthread_mutex_lock(&g_view.persist_lock);
	const bool persisted = drm_output_view_persist_locked(name);
	pthread_mutex_unlock(&g_view.persist_lock);

	if (persisted)
		blog(LOG_INFO, "drm-output: view %s -> %s (persisted)", drm_output_view_name(prev), name);
	else
		blog(LOG_WARNING, "drm-output: view %s -> %s (NOT persisted -- no readable drm-output config)",
		     drm_output_view_name(prev), name);
	return persisted;
}

/* Render the registered Multiview into a claimed scanout buffer. Graphics thread, context held. */
static void drm_output_view_render_multiview(void)
{
	const int idx = drm_output_claim_render_buf();
	if (idx < 0)
		return;
	gs_texture_t *tex = drm_output_render_buf_texture(idx);
	if (!tex)
		return; /* a claimed buffer that is never published simply stays role-free */
	uint32_t w, h;
	...
	/* The same frame the display path gives a projector's draw callback: black clear, depth off,
	 * no culling, viewport + ortho over the whole target. The Multiview letterboxes the canvas
	 * aspect into it itself. */
	...
	/* Submit before publish: the kernel page-flip waits on the BO's implicit fence. */
	gs_flush();
	...
	if (!g_view.bind_live_logged) {
		...
		blog(LOG_INFO,
		     "drm-output: multiview bind LIVE (built-in Multiview rendered into the scanout buffer at "
		     "%ux%u)",
		     w, h);
	}
}
