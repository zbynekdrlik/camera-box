				    ...
				    double *applied_ppm_out)
{
	...
	/* camera-box #960: a block whose instantaneous ppm magnitude clears the sanity ceiling
	 * carries no real timing information (starved/bursting source, not clock drift) --
	 * REJECT it: no EMA update, no elapsed_lock_s credit, no slew step, keep applying whatever
	 * applied_ppm was already in effect. Mirror of the Rust reference's identical guard -- keep
	 * numerically identical (the two branches below update the EXACT SAME fields the pre-#960
	 * code always updated unconditionally; only the estimate/lock/slew computation is skipped). */
	if (fabs(instantaneous_ppm) > ASRC_MAX_SANE_INSTANTANEOUS_PPM) {
		c->starved_block_count++;
	} else {
		/* TIME-based EMA smoothing factor: alpha = 1 - exp(-block/tau), so convergence speed
		 * is independent of how the caller chunks audio callbacks (a real device may deliver
		 ...
		 * still-converging estimate (camera-box #806: the outer-loop bias is folded in HERE,
		 * so it is just as inert as the inner estimate before lock -- never applied on its
		 * own). Once locked, add the outer-loop bias to the inner estimate and clamp the SUM
		 * to the hard ppm bound before ever using it as a target. Mirror of
		 * src/asrc_bench.rs RealtimeAsrcCompensator::compensate. */
		const double target_ppm = (c->elapsed_lock_s < ASRC_MIN_LOCK_S)
						   ? 0.0
						   : asrc_clamp(c->estimated_ppm + c->outer_bias_ppm,
								-ASRC_MAX_PPM, ASRC_MAX_PPM);

		/* Slew-limit the APPLIED correction toward the target -- caps how fast the
		 * resample-ratio nudge may change, independent of how fast the estimate itself
		 * moves. */
	 ...
	 * both the starved and healthy branch above) so the ~60s log cadence never goes silent
	 * during a sustained starve -- exactly the moment the new starved_blocks=N telemetry is
	 * most needed. Uses whatever applied_ppm is in effect (held unchanged on the starved
	 * branch), so it still reports the real correction being applied to this real audio. */
