...
struct obs_source {
	...
	uint64_t genlock_n1_grows;          /* camera-box issue 1367: cumulative N==1 DEPTH HOLDS -- genlock_should_hold_n1_phase held one STEADY tick because a deep N==1 conveyor sat shallower than its pin-derived depth (base + 1 frames). Printed as n1_grows=; audit-line-only. Zeroed at create (bzalloc). Decision authority: src/genlock_n1_depth.rs should_hold_n1_phase. */
	...
	int genlock_audio_hold_mode;                /* camera-box issue 1367: the audio hold mode last applied (GENLOCK_AUDIO_HOLD_OFF / _LATENCY / _TIMECODE); audio thread; a change forces a re-placement + an ASRC level-target shift. Zeroed at create. */
	...
	bool genlock_shallow_capped;                /* the last latch was capped by the min-latency (imag) guard */
