...
struct obs_source {
	...
	uint64_t genlock_relocks;          /* camera-box #401: drift-guard re-locks — the cadence lagged the live edge past relock_drift_ns (2*interval + interval/4; a stall's backlog landed / the stream jumped) and jumped to the newest due frame (catch-up keeps the IMAG latency contract). Mirror of CadenceOutcome::relocked. */
