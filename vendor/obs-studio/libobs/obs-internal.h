...
struct obs_source {
	...
	uint32_t genlock_excess_run_ticks; /* camera-box #1355: consecutive plain-N==1 STEADY render ticks the queue has sat MORE than GENLOCK_DRAIN_SUSTAIN_HYSTERESIS_FRAMES (1) above the CEIL drain target — the SUSTAINED-EXCESS window that discriminates the drift-gained frame (persists tens of minutes) from ordinary arrival jitter (one or two ticks). The stream box's deep N==1 NDI 2ME PGM FIFO gains one frame every ~40 min from the +13 ppm wall-vs-QPC clock drift (issue-1355): the video release keys on the DanteSync WALL clock, the render tick consumes on the free-running QPC. The #859 depth drain only fires at depth > target + 2 and the #1049 phase converge is N>=2-only, so this persistent ONE-frame excess has no other corrective. Incremented while the excess holds on the drain_eligible path, reset to 0 the moment it does not AND on a sustain shed; also zeroed at ACQUIRE alongside genlock_ticks_since_drain. Zeroed at create (bzalloc). Decision authority: src/genlock_backlog.rs should_sustain_drain / DRAIN_SUSTAIN_TICKS (Tier-0 unit-tested + the C-vs-Rust parity gate). */
	uint32_t genlock_sustain_sheds; /* camera-box #1355: cumulative SUSTAINED-EXCESS one-frame drains (genlock_sustain_drain_due fired to shed the drift-gained frame on the N==1 deep FIFO). A sustain shed ALSO counts into genlock_dropped_due like every other drop; this distinguishes it so the post-deploy >=1 h audit can see the drain fire ~once per drift frame (~40 min) and correlate it with wall_qpc_drift_ms. Printed at the END of the 5s audit line (sustain_sheds=, mutually non-substring with converge_sheds). Zeroed at create (bzalloc). */
