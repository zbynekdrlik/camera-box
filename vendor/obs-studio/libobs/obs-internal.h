...
struct obs_source {
	...
	uint64_t genlock_n1_grows;          /* camera-box issue 1367: cumulative N==1 DEPTH HOLDS -- genlock_should_hold_n1_phase held one STEADY tick because a deep N==1 conveyor sat shallower than its pin-derived depth (base + 1 frames). Printed as n1_grows=; audit-line-only. Zeroed at create (bzalloc). Decision authority: src/genlock_backlog.rs should_hold_n1_phase. */
