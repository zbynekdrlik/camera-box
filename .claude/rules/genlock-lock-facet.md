...
| Gather parser | `scripts/bundle_state_gather.py` (`genlock_lock_facet_from_log`) + wired in `scripts/bundle-state-server.py` | Reshapes the newest `genlock-lock-json:` line into the nested facet `{state, reason, n_inputs, n_locked, n_absent, latency_ms, recent_event, qpc_drift_ms, clock:{state}, output:{present, stamping_wallclock}, inputs:{<name>:{locked, connected, ...}}, source:"log"}`. Attached AFTER `build_bundle_state` (a nested object, NOT a flat version-integrity string). OMIT-when-absent (a stock OBS → no facet, never a false UNLOCKED). Reuses the already-bounded log_text (no second read → no new #1222 cache). #1299 (schema v2) added `n_absent` (senderless input count) + per-input `connected`; #1299 Part 3 (schema v3) added `recent_event_inputs:[{name, events}]` (the top recent-event offender, omit-when-absent). #1303 (schema v4) added `audio_unexpected_inputs:[{name}]` (a silent-by-contract source found AUDIBLE — the double-audio hazard; names-only, omit-when-absent), enriched to `reason=audio_unexpected:<name>` by `genlock_lock_decision.analyze`. #1299 Part 4 (schema v5) added report-only `qpc_drift_ppm` / `qpc_expected_ppm` / `qpc_step` (the windowed wall-vs-QPC drift telemetry; since #1357 the verdict keys on the wall STEP only — `qpc_step` — never on the rate or the cumulative `qpc_drift_ms`). #1341 (schema v6) added top-level `n_idle` (connected-but-keep-alive-only input count) + per-input `idle` — an idle SongPlayer playlist input excluded from the DEGRADED gate (see `genlock-lock-indicator.md`); `analyze` carries `n_idle` for the card/log text (verdict unchanged, it keys on the widget's `state`). Issue 1372 part D (schema v7) added the `media_clock:{state, drift_us, window_s, ready, discipline}` object (the audio clock term, see `genlock-lock-indicator.md`; `drift_us` is 0 while `ready` is false — the widget does not publish a filling window's rate), omit-when-absent or malformed; `analyze` enriches `reason` to `media_clock:drift` / `media_clock:undisciplined` and prints `media_clock` / `media_clock_drift_us` / `media_clock_discipline`, which the watchdog logs every pass. All degrade gracefully for an older line (`n_absent`/`n_idle`→None, `connected`→True, `idle`→False, the `qpc_*_ppm` trio→None, no `recent_event_inputs`/`audio_unexpected_inputs` key). |
| Box roster | `scripts/lib/obs-fleet.sh` `genlock-lock` facet = `strih-lx stream imag resolume` (issue 1317: strih-lx is the production strih since M4; imag is `retired` and dropped centrally) | imag is a pure receiver that still locks every input → IN scope when provisioned. resolume is paged only while `obs_fleet_is_home` (traveling). |
...
- **#1299 Part 4 + #1357 — the `qpc_drift` verdict is the wall STEP only, and its rate telemetry is
  report-only.** The widget decides `state` from the pure `genlock_qpc_drift_beyond_bound` (a wall
  STEP > 33 ms, the same meaning on every box — see `genlock-lock-indicator.md` for why neither the
  cumulative offset nor a rate may gate), so neither a steady disciplined slew (Windows) nor a
  dantesync `f_ptp+f_phase` excursion (Linux, where the measured rate is 0 by construction) DEGRADEs.
  The facet carries `qpc_drift_ppm` (measured), `qpc_expected_ppm` (the reported slew), `qpc_step` and
  the raw cumulative `qpc_drift_ms` — ALL report-only; the watchdog only pages on the carried
  `state`, and logs `qpc_drift_ppm`/`qpc_expected_ppm` per pass for observability (the `analyze`
  output carries them alongside `n_absent`).
