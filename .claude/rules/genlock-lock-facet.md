...
- **#1299 Part 4 — the `qpc_drift` verdict is a WINDOWED RATE + STEP, and its telemetry is
  report-only.** The widget decides `state` from the pure `genlock_qpc_drift_beyond_bound` (a STEP >
  33 ms, or a rate off the dantesync-reported `f_ptp+f_phase` slew by > 50 ppm over a 300 s window —
  see `genlock-lock-indicator.md`), so a steady disciplined slew no longer chronically DEGRADEs. The
  facet carries `qpc_drift_ppm` (measured), `qpc_expected_ppm` (the reported slew), `qpc_step` and
