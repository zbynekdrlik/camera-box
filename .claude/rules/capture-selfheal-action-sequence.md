...
**Attribution/watchdog scope (#1193/#1200, mirrors #1128):** the over-rate AND latch-halving reset lines carry distinct tags (`#1193 over-rate self-heal` / `#1200 latch-halving self-heal`), so — like the #1128 reset — neither is matched by `self_heal_reset_grep_pattern` (`#663 self-heal:`) or `capture_rate_defect_grep_pattern_hard`. Threading each into the dev1 self-heal-attribution table (`restart_event_kind_patterns`) + a per-trigger dev1 alert watchdog (the #1128 pattern) is a FOLLOW-UP for when the respective canary graduates — not shipped here, since both resets ship default-OFF (never fire unattended), so the attribution gap is cosmetic while the canaries are armed. (Note: `frozen_leg`/`self_heal_reset` were RESTORED to blocking by issue 905 item 2 — no longer the #914 report-only state — so once a canary graduates to default-ON its attribution wiring becomes load-bearing, not cosmetic.)
...
`cargo fmt --all --check` (parses the whole crate) + a standalone `rustc --edition 2021 --test` replica of the builders + a byte-fidelity diff of builder output vs the pre-change strings extracted from git. `capture_rate_selfheal` is a DEFAULT-feature module, so its `#[cfg(test)]` tests DO run on CI `cargo test`. CI is also the FIRST `clippy -D warnings` run, and a plain `rustc` replica does not run clippy — hand-audit new code for the two heuristics that have actually bitten this area: `assertions_on_constants` (a runtime `assert!` over consts in a test — the #1200 🔴; use `const _: () = assert!(...)` instead) and `nonminimal_bool` (a `!a || !b` gate — the #1201 🟡; write `!(a && b)`).
