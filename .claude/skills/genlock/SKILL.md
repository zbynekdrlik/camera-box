...
## #266 — NDI-receive stuck-state watchdog (`scripts/stuck-watchdog.py`)

Detects the #265 collapse (cam→OBS NDI receive falls to ~7-10 fps / freezes; genlock FIFO STARVED;
intermittently `dantesync.exe` runs away ~1 core; fixed only by a PC reboot) and alerts on Discord
via `airuleset notify`, so the operator catches it BEFORE a show. Read-only Python (stdlib), Tier-0,
no rig deploy. Unit-tested in `tests/python/test_stuck_watchdog.py` (CI `python -m pytest tests/python`).

**Where the audit lives (real-rig gotchas — cost time to find):**
- The broadcast OBS is the one launched from `C:\Program Files\obs-studio\bin\64bit\obs64.exe`; its
  log is `%APPDATA%\obs-studio\logs\<latest>.txt` (NOT a D:\_APPS portable install). Find it via
  `Get-ChildItem "$env:APPDATA\obs-studio\logs\*.txt" | sort LastWriteTime -desc | select -first 1`.
- **`LastWriteTime` can read STALE** while OBS holds the file open — the file IS being appended; trust
  the newest `genlock-fifo audit` line's HH:MM:SS, not the mtime.
- A genlock-fifo audit line is written (~5 s) ONLY while the source is on an actively-rendered scene.
  When a source leaves program OBS STOPS logging it → its last samples freeze in place and go OLD.
  (Verified 2026-06-26: cam1=`NDI cam5` ran clean 30 fps post-reboot, collapsed+froze at 20:11
  (received stuck 41421, underruns climbing ~30/s, depth=0, overruns=0), then the source left program
  and logging stopped at 20:13:56 — the frozen tail sat there for hours.)
- The #265 audit signature: `received` frozen/slow + `underruns` climbing (~30/s) + `depth=0` +
  `empty_run` climbing + `overruns=0`. The "repeat then jump" stutter = `received` plateaus then jumps.
- Audit format has `holds=` between `underruns=` and `overruns=` (added by #148) → parse field-by-field
  (`\bkey=\d+`), never positionally.
- dantesync CPU: `ListProcesses` filter `dantesync` (the runaway is intermittent — it read 0% on the
  healthy post-reboot box; the watchdog checks it INDEPENDENTLY of the NDI audit).

**The discrimination (why the first cut was rejected over 14+ findings — get this right):**
Read a WINDOW of audit samples per source (default 60 ≈ 5 min), then per source:
- STUCK (alert) = was HEALTHY (`peak` received-fps ≥ fps_floor SOMEWHERE in the window — the baseline)
  AND NOW collapsed (recent fps < fps_floor, down to a full 0-fps freeze) AND starved (recent
  underruns climbing > rate AND window-level overruns NOT dominant AND recent depth ≤ depth_floor).
- benign IDLE (quiet) = never delivered (peak ~0) OR steadily-low (never WAS ≥ fps_floor, so never
  "collapsed" — a sub-30 source on a 30 fps genlock naturally underruns) OR overrun-dominant (holding).
- The **peak-fps baseline** is the collapsed-vs-always-low discriminator; **overruns** is the
  holding-vs-starved discriminator. Do NOT use the absolute cumulative `received` counter as the
  "was delivering" signal — it can't tell a steady-low feed from a collapse (delta matters, not
  absolute). Window must be large enough that the healthy lead-in stays in view while a collapse
  persists; a freeze outliving the window is left to the operator already alerted at onset.
- NAMED `--source` input: stricter — alerts on ANY below-floor receive; ABSENT or STALE → `vanished`
  (a single FRESH sample is "just appeared", NOT vanished).
- STALENESS: pass `--now HH:MM:SS` = the **box** clock (read via the same win-* MCP as the log). A
  source whose newest sample is older than `--max-sample-age` (default 60 s) stopped logging → skipped
  (named → vanished). `is_stale` is `age > max_age` (a future-dated sample = box clock ahead of the
  poller = freshest data, NEVER stale — never `abs(age)`, which would flip a live stuck box to stale).

**Exit codes:** 0 healthy / 1 alert+delivered / 2 no-signal / 3 alert+delivery-FAILED. `--dry-run`
prints the alert instead of sending (use it for verification — do NOT fire a real Discord alert).

**Run recipe (per box, from the win-* MCP poller):** read the latest OBS log + dantesync CPU% + the
box's current HH:MM:SS, then
`python3 scripts/stuck-watchdog.py --box strih --host 10.77.9.202 --obs-log <log> --dantesync-cpu <pct> --now <HH:MM:SS>`.
Recovery on a real alert = reboot that PC (graceful OBS stop first; see ops/obs-ops skills).
