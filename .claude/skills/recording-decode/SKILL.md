...
2. **`node_render_step` returns 1 for strih/stream** (gap-ignore). A delivered frame MISSING its
   ...
   + cam1/cam3/cam4 (per-emitted). The `burn_contiguity_in_window_with_step` PerRenderTick step≥2
   capability + its #11 tests are RETAINED for a genuinely-clean-decimation per-render hop. **#571
   (below) additionally made the PerEmittedFrame (cam1/cam3/cam4) branch consult `step` — as a
   DECIMATED-HOP discriminator (step≥2 ⇒ cam forward gaps are decimation, never charged), NOT as
   excess-gap math** (this #360 fix itself is unchanged for strih/stream).
...
- cam1/cam3/cam4 (PerEmittedFrame) — pre-#571 this was UNAFFECTED (its set-based real-drop detection
  ignored `step` entirely, always scanning the raw `min..=max` integer span). **#571 made `step` its
  DECIMATED-HOP discriminator** (see the dedicated section below): `expected_step >= 2` ⇒ the hop is
  a configured decimation and cam forward gaps are NOT charged at all (NO excess-gap math — the real
  beat defeats it, see below); `expected_step == 1` ⇒ the exact pre-#571 raw `min..=max` scan, so a
  non-decimated PerEmittedFrame caller is unaffected (never-mask preserved).
...
historical design; its live value stays 1.) **#571 uses `step` on cam1/cam3/cam4 as the
DECIMATED-HOP discriminator (gap-ignore when ≥2)** — see the dedicated section below.

## #571 — DECIMATED cam(60fps)→strih(30fps) hop: cam forward gaps are decimation, NEVER charged (no gap math)
...
or cam3/cam4's equivalents) is emitted at 60fps but lands in strih's 30fps recording DECIMATED —
strih samples only ~half of the emitted ids. Before #571 the PerEmittedFrame set-based scan (which
never consulted `step`) charged EVERY decimated-away id as `real_drop` — 11087 phantom drops on run
554307 while strih's OWN node burn (911002) was fully contiguous (strih dropped NOTHING).

**THE VALIDATED DATA MODEL (run 554307 partials — the load-bearing fact):** cam1's 911001 ids in
strih's recording are strictly monotone (0 backward jumps), have **0 Nones**, and their
forward-delta histogram is `{0: 2 (benign dup), 1: 9256, 6: 34, 7: 1767, 8: 45}` — bursts of ~5
consecutive ids (delta 1) then a jump of ~7, mean 1.998 ≈ 2. strih's 911002 (PerRenderTick,
gap-ignored, judged zero-loss) has the SAME beat. Consequence: **NO gap-size math can separate
decimation from loss on this hop.** An observed-cadence (mode) step reads the dominant delta 1 and
charges every jump (attempt #2's failure — the re-derived 11087); a hardcoded step-2 grid masks
genuine step-1 drops (attempt #1's failure, the #356/#208 never-mask break). The correct
discriminator is WHETHER THE HOP IS CONFIGURED DECIMATED, not the gap size.

Fix (`src/probe/burn_contiguity.rs` `burn_contiguity_in_window_with_step`, PerEmittedFrame branch):
the forward-gap candidate generation (the `(lo..=hi)` scan + the #216/#226 interstitial budget) runs
**ONLY when `expected_step <= 1` (a 1:1 hop) — byte-identical pre-#571 behavior there**. On a
DECIMATED hop (`expected_step >= 2`) it is SKIPPED ENTIRELY: no forward-gap candidates exist.
Genuine loss on the decimated hop is still caught — NOTHING weakened, the invalid signal is
replaced by the valid ones: a delivered frame with NO readable cam burn (`None` → BURN-UNREADABLE,
still fails the gate), a backward jump/corruption, strih's OWN 911002 burn (its per-render check is
the strih-drop detector), the cam2→cam1 OPTICAL tick (1:1, non-decimated), and the frozen-camera
gate (#365). The `unreadable_consumed` bookkeeping stays the plain pre-#571 `prev + nones` (only
the 1:1 scan consumes it).

`recording-verdict.rs` `node_render_step` branches on `CAMERA_UNDER_TEST_NODES` (cam1/cam3/cam4):
returns `recording_span_gate::painted_tick_step(refresh_hz, capture_fps)` (`round(refresh/capture)`,
floor 1; = 2 on the rig: refresh 60 / capture 30) — the DECIMATED-HOP discriminator, not an
excess-gap step. strih/stream are unchanged (still gap-ignore, #360). `extract_partial_flagged_
frames` derives the SAME cam1 step so on-box pixel-proof flagging matches the merge.

RED→GREEN lock: `src/probe/burn_contiguity.rs` `#571` test section
(`in_window_cam1_decimation_step2_clean_matches_run_554307_signature_571` — the REAL delta-1-bursts
+ ~7-jumps beat → zero loss; `..._forward_gap_of_any_size_is_not_charged_571`;
`..._none_amid_wide_gap_charges_only_the_none_571`; `..._unreadable_burn_is_not_double_counted_...`;
`in_window_cam1_step1_hop_forward_gap_is_still_a_real_drop_571` — the 1:1 never-mask). Plus
`recording-verdict.rs`: `node_render_step_is_decimation_aware_for_camera_under_test_nodes_571`,
`node_verdict_cam1_decimation_step2_forward_gap_not_charged_and_none_fails_571`,
`cam1_decimated_forward_gap_is_zero_loss_but_1to1_gap_stays_real_drop_356` (+ the `window_none`
BurnUnreadable tests, and the `--capture-fps 60` 1:1 reconciliation locks in the `_356`/`_24`
tests).
