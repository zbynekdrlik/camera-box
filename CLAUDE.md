...
- strih on-box NIC-fail self-heal watcher (#1199 — SYSTEM schtasks every 2 min: on a confirmed total LAN outage → best-effort OBS-WS stop → graceful `shutdown /r`, until the card is replaced; owner ruling — NO adapter disable/enable/restart, it HANGS on strih; trigger is REACHABILITY not adapter status (the NIC read Up while dropping packets); reboot gated on state-persist + capped at MaxReboots; pure `strih_nic_selfheal_decision.py` mirror + static-validated `.ps1` since no pwsh on dev1 CI; complements the alert-only #1001 reach watchdog) → `.claude/rules/strih-nic-selfheal.md` (auto-loads on its `paths:`)
