...
- imag active-set NDI idle policy (#1218 — imag idles INACTIVE cameras' `NDI CAM{n}` receivers so it stops decoding them and thermal-throttling; the ONE `enforce_ndi_active_policy` point + pure `desired_ndi_state`; the #1158 deliberate-idle-vs-wedge discriminator; `--active-cams`/state-file resolution; the on-box lazy-`obs_phase2`-import safety, the `verify_parity` grep-qxF whole-line rule, passwordless imag WS, and the #675 sourced-lib E2E/rig wiring) → `.claude/rules/imag-ndi-active-set-policy.md` (auto-loads on its `paths:`)
