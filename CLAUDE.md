...
- Realtime isolation subsystem (merged RT-conditional affinity.rs capture-IRQ routing + verify (ac); the STAGED PREEMPT_RT kernel choice — reboot-class supervisor-only via the rt-kernel-upgrade.sh dry-run planner + runbook; safe atomic order reboot-before-purge; per-box GRUB drift + Pro-not-attached blocker; defect 2 = runbook Step B; #899) → `.claude/rules/realtime-isolation.md` (auto-loads on its `paths:`)
