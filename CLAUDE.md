...
- Early-gate PIN doctrine (EVERY early version/preflight gate PINS to the expected release + fails CLOSED on UNKNOWN; peer parity is a supplement, never a substitute; the MOVING-pin technique for a continuously-deployed component = read origin/main + auto-deploy so pin and fleet advance together; the live audit table + open holes #1137 OBS-bundle-vs-vendor / #1138 frame-probe-unpinnable; #1136) → `.claude/rules/early-gate-pin-doctrine.md` (auto-loads on its `paths:`)
