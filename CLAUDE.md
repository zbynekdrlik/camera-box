...
- RESOLUME-SNV cg OBS on the genlock fleet (the `resolume` arm in deploy-genlock-fleet.sh / launch-obs-genlock.sh: win-resolume MCP, has_ahk=1 (AHK v2 safe-loop respawns Arena+OBS — stop→deploy/relaunch→verify→restart-AHK, SessionId gate over obs64+AutoHotkey64, per-box .ahk identity prefers the Startup .lnk; #1295), hostname-not-pinned-IP + live identity-confirm, explicit-only never the empty-default fleet; the genlock build DEFAULTS genlock_fifo=true + latency 3 + certified coercion so the "pin" step is a CONFIRM not a WS write; the `_ndi_inputs_matching` sp-*_video prefix-floor sentinel in latency_pins_verify.py/latency-pins-baseline.json; NDI output `RESOLUME-SNV (cg-obs)` + #1185 :5961 pin; the supervisor deploy→relaunch→confirm→cg-chain-verify runbook; #1295) → `.claude/rules/resolume-cg-obs.md` (auto-loads on its `paths:`)
...
- CG-path burn-id node role (SongPlayer 911014 chain-ORIGIN painted by songplayer#151 — never in `CAMERA_UNDER_TEST_NODES`; cg OBS 911015 HOP at `burn_geom::Corner::BottomCenterRight` via the `resolume` host-role; both tick-excluded + `all_burns`-excluded; the crate-root `src/cg_chain_gate.rs` per-hop contiguity+hold REPORT-ONLY seam that never touches the camera-chain `overall_pass`; the opt-in `CG_CHAIN=1` leak-guarded E2E profile; the GENERATED decode fixture a real cg-OBS frame must replace once songplayer#151 ships; #1301) → `.claude/rules/cg-burn-node-role.md` (auto-loads on its `paths:`)
