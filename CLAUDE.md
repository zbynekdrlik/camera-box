...
- Projection-tap scanout-TEAR detector (issue 781 + the issue-1196 v2 aux Vernier tick pair — cam2's grabber captures imag-nb's HDMI, so cam2's sweep leg IS the projection path; tear = UNION span of the primary dual-QR + the bottom aux tick pair > 1 (v2 cures the single-band blindness), merge-side, no schema bump; `AUX_TICK_RUN_ID` 911013 in `NODE_BURN_RUN_IDS` + the python mirrors; STILL report-only — promotion needs the mined real-frame fixture + a known-torn calibration run, preconditions in the rule) → `.claude/rules/projection-tap-tear-detect.md` (auto-loads on its `paths:`)
