...
- strih on-box NIC-fail self-heal watcher (#1199 — SYSTEM schtasks every 2 min: Restart-NetAdapter → graceful `shutdown /r` on a confirmed total LAN outage, until the card is replaced; trigger is REACHABILITY not adapter status (the NIC read Up while dropping packets); pure `strih_nic_selfheal_decision.py` mirror + static-validated `.ps1` since no pwsh on dev1 CI; best-effort OBS-WS stop; complements the alert-only #1001 reach watchdog) → `.claude/rules/strih-nic-selfheal.md` (auto-loads on its `paths:`)
