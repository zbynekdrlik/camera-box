...
- N==1 PIN-DERIVED DEPTH (issue 1367: a deep N==1 input like the stream `NDI 2ME PGM` settles on `base + 1` frames after EVERY restart — `base = ceil((pin − 1 µs)/interval)`; shed/hold one frame on the ROUNDED presented age read at the render tick's SCHEDULED instant (`video_time`, never the processing wall — `video_sleep` catches late ticks up), deep sources only; `genlock_phase_converge_due` untouched, the SOURCE wrapper routes N==1; `n1_grows=` audit field; the early-phase and queue-length-drain limits; the Tier-0 + mutation recipe) → `.claude/rules/genlock-n1-pin-derived-depth.md` (auto-loads on its `paths:`)
