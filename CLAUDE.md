...
- Per-camera NDI SEND stagger (`src/send_stagger.rs` decides the offset: camN sends (N-1)×1.2 ms after the emit gate so the seven per-tick bursts stop overflowing the strih-lx 2.5 GbE `foh1_video ether2` port, ≈48 vs ≈790 drops/min; N from the OS hostname CAM<N> via one pure `plan()`, clamped to 45 % of the SHORTER of the send/capture interval, `STAGGER_ACTIVE` = the one-constant rollback; the WAIT lives on the send thread, NEVER in the capture loop — `src/send_handoff.rs` single newest-wins slot + absolute monotonic deadline (emit-gate decision + offset) + expedite-on-newer + the 5 s `#1242 send stagger:` line (capture work vs interval, lateness, send time, REPLACED, slot use; replaced frames leave the sent count) on every box; `src/ndi_send_thread.rs` (SCHED_FIFO one step BELOW capture, so capture always preempts a send) owns the NdiSender, the synchronous send (buffer recycled only after it returns), the #944 heartbeat and the #297 re-announce; one job per emitted iteration carries the starvation repeats + the current frame; the burn thread waits for the same deadline; timecode + emit grid untouched; the 1 GbE wire-time vs 2.5 G drain arithmetic + the live acceptance steps; issue 1242) → `.claude/rules/ndi-send-stagger.md` (auto-loads on its `paths:`)
