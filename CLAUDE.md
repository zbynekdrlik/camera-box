...
- Burn-id contiguity is PRESENCE-ONLY (set-based, blind to a REPEATING/freezing hop) — the per-hop max-hold term is the separate report-only `src/burn_hold.rs` (`MAX_HOLD_FRAMES=4`, flip-to-LIVE #870); any repeat/run-length metric MUST use `burn_ids_with_frame_index_in` + break on non-adjacent `frame_index`, never `burn_ids_in` → `.claude/rules/burn-hold-uniqueness.md` (auto-loads on its `paths:`)
...
- imag leg recording verdict report-only seam (issue 798): the imag leg gates `all_pass` in TWO places (whole-recording node fold + all-cambox per-segment sweep), both routed through the ONE `imag_leg_gate::gates_overall_pass()` seam — the follow-up flips ONE fn to make both blocking (never the per-cambox stream fold); the imag verdict only FLOWS when recording-e2e.sh `[8/8c]` succeeds (0/76 historically), so check `full_chain.imag_leg_verified` + the IMAG-LEG-VERIFIED marker before trusting a green run; calibrate before flipping blocking → `.claude/rules/imag-leg-report-only.md` (auto-loads on its `paths:`)
...
- MV-fps observability + live alarm (the `multiview-audit:` emit → `mv_audit.rs`/`mv-fps-gate` → dev1 `mv-fps-alert-watchdog.sh`; DATA-FIRST floor calibration mining live `rendered_fps` per box; the `canvas/2−tol` tight-for-divisor2/loose-for-divisor1 asymmetry; strih 4K MV collapses under CONTENTION not itself; autostart-restart-reset via log-id change; the `cargo build --release`-in-comment Tier-0-hook trap, #771/#1083) → `.claude/rules/mv-fps-watchdog.md` (auto-loads on its `paths:`)
