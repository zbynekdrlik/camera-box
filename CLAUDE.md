...
- Windows OBS media clock = the dantesync-disciplined tick (issue 1372 part A: `os_gettime_ns()` in `platform-windows.c` integrates QPC at the system-time rate `inc / adj` from GetSystemTimeAdjustmentPrecise -- a LARGER adj is SLOWER, measured live; units are QPC counts; resolved at runtime from kernelbase.dll, kernel32 does not export it; rebase only on a rate change, 250 ms poll, one poller, QPC read under the shared SRW lock, 1000 ppm clamp, rate 1 without dantesync; `os_sleepto_ns` waits on the same clock, never raw QPC counts; the Tier-0 authority `src/os_clock_discipline.rs` + the lift-onto-a-fake-Win32-layer parity gate + the mutation proof; live check = `wall_qpc_drift_ms` goes flat) → `.claude/rules/windows-disciplined-media-clock.md` (auto-loads on its `paths:`)
