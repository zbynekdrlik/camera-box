...
- Projection-tap scanout-TEAR detector (issue 781 — cam2's grabber captures imag-nb's HDMI, so cam2's sweep leg IS the projection path; the payload-level "≥2 paint generations in one frame" tear signal via the dual-QR Vernier span, computed merge-side from the carried payloads with no schema bump; PROVEN-BLIND on the current single-vertical-band content so report-only + a `TearSignalViability` promotion gate; a viable LIVE gate needs painter VERTICAL tick redundancy) → `.claude/rules/projection-tap-tear-detect.md` (auto-loads on its `paths:`)
