...
- NDI discovery = the dev1 NDI Discovery Server + ONE output per cambox (issue 1342: the unconsumed `CAMn (30p)` stream removed, setup-device deletes its drop-in; the ONE client-config source of truth `scripts/lib/ndi-discovery.sh` (`networks.discovery` = dev1, `networks.ips` empty) written by setup-device STEP 7 (/etc/ndi + the camera-box `NDI_CONFIG_DIR` drop-in, ProtectHome hides /root/.ndi) and setup-strih step 4b, graded by verify-device `(an)` / verify-strih item 34; the dev1 `ndi-discovery-server` --user unit ships DISABLED; the BOM-free Windows laptop `.ps1` + checked-in JSON; a configured SENDER stops mDNS, so the client config is written only behind the `NDI_DISCOVERY_ENABLED` rollout gate (default off, flipped by the supervisor after the server + every receiver are ready; the stock displays / guest laptops / port-map audit need a plan first)) → `.claude/rules/ndi-discovery.md` (auto-loads on its `paths:`)
