...
- In-process USB self-heal action SEQUENCE — ONE helper `capture_rate_selfheal::attempt_self_heal(...)` that ALL FOUR triggers call (#656/#663/#971 capture-rate, #1128 grabber-STUCK, #1193 sustained OVER-RATE, #1200 LATCH-HALVING; add a 5th via a `SelfHealMessages` const, never a re-inlined copy); each keys on a DIFFERENT capture signature (see the four-trigger discriminator-band table in the rule); emitted log lines are BYTE-anchored by dev1-watchdog grep patterns (harness_capture_rate_guard / harness_self_heal_attribution_895 test scripts/lib patterns vs hardcoded samples — neither reads main.rs), so preserve substrings; the real reset is injected for Tier-0 testability, #1149/#1193/#1200) → `.claude/rules/capture-selfheal-action-sequence.md` (auto-loads on its `paths:`)
