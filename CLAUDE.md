...
- Reading the stream `asrc: source 'mbc' estimated=` residual (a steady ≈+8 ppm is the Dante-GM-vs-UTC FLOOR mirrored by dantesync's `[PHASE-SLEW] f_phase=` DC — `|estimated| ≈ |f_phase|` is the cross-check, never "DVS still off the GM"; a steady ≈−18 ppm was the DVS/PTP port-collision signature fixed in dantesync 1.8.53 — re-check `Get-NetUDPEndpoint -LocalPort 319,320` = `ptp.exe` first; canary-only upgrade ⇒ bump `DANTESYNC_VERSION_PIN` with the fleet roll; dantesync#109 / #1265) → `.claude/rules/asrc-residual-floor.md` (auto-loads on its `paths:`)
