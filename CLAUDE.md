...
- E2E `[0/8]` test-camera shutter/ISO ENFORCE (issue 1371: read the ONE BMPCC over gphoto2 on the relay-paused cambox holding its USB-C, set what differs from `scripts/camera-test-baseline.json`, read back, abort on MISMATCH; the relay's own key names `iso`/`d002` = shutter ANGLE x100; absent camera + null baseline = loud report-only UNVERIFIED, once pinned an absent camera aborts; ack name `testcam`; the supervisor pin runbook; the owner's PRODUCTION exposure snapshotted before the first set of a development period and restored + read back by `rig-mode.sh event`, never fatal to it, reported by the handover check) → `.claude/rules/camera-test-settings.md` (auto-loads on its `paths:`)
- strih-lx intercom hub (issue 1345: the `intercom/vban` shared codec extraction + `intercom/hub` N-1 mix-minus daemon replacing VB-Matrix for the VBAN camboxes; the cambox `camN` both-ways wire contract kept byte-identical; the structurally-enforced N-1 invariant; `scripts/vbmatrix_to_intercom_toml.py` + the byte-for-byte `intercom.strih-lx.toml` parity pin; the enable-only systemd unit + the "never send VBAN to a real cambox while Windows strih is live" rule; the M1/M2/M3/M4 map + the Tier-0 verify path; the phone PWA UX — auto-join receive-only, live meters, in-place mic renegotiation around the janus.js 1.1.2 replace-path crash — and its real-janus.js Playwright E2E) → `.claude/rules/strih-intercom.md` (auto-loads on its `paths:`)
...
- Recording-verdict QR decode path (fast/robust gate — the core is `src/probe/recording_decode.rs` since issue 1374, `qr.rs` keeps the primitives and re-exports it; per-recording burn sets, #186 fixtures) → load `.claude/skills/recording-decode`
- Burn-isolated slot recovery in the recording decode (issue 1370: a missing expected node burn is decoded from its own `src/burn_regions.rs` slot crop so optical content the camera puts into the #202 bottom tile cannot hide it; only missing ids merged, never optical/aux; `burn_regions` is the ONE Rust burn geometry (colour_sample pads its slots), pinned to the shipped burn-geom.hpp by a default-feature C++ parity test; the local real-rqrr rustc harness + the PIL-resize-is-not-CatmullRom trap; issue 1367 echo gate: a node burn counts only when read inside its own slot + pad, every recording decode drops every other read of it before reads merge (a camera filming an OBS multiview), report-only `burn_echoes_rejected`, synthetic grouped-decode tests must draw burns at their real slot) → `.claude/rules/burn-region-recovery.md` (auto-loads on its `paths:`)
- CG-chain receiver-side verdict (`scripts/cg-chain-verify.sh` + pure `scripts/lib/cg-chain-verify.sh`: per-hop SongPlayer→cg OBS→strih/stream genlock-fifo playback verdict REPLICA pinned to `resolume_playback::evaluate` by a parity harness + asrc residual band + soak CSV; exits non-zero on FAIL; rig-health report-only NOTE row; accepts songplayer 146-151 from the RECEIVER side per issue-1294 §8, pixel level is issue 1301; #1300) → `.claude/rules/cg-chain-verify.md` (auto-loads on its `paths:`)
- RESOLUME-SNV cg OBS on the genlock fleet (the `resolume` arm in deploy-genlock-fleet.sh / launch-obs-genlock.sh: win-resolume MCP, AHK mode `guard` (the owner's AHK v2 safe-loop respawns Arena+OBS — issue 1372 ruling: stop it only if running, NEVER start/restart it, AutoHotkey64 count report-only, OBS launched directly + verified by one live obs64 and the `build <sha> - <label>: cg` title (label localized); the old #1295 stop→restart-AHK contract is retired), hostname-not-pinned-IP + live identity-confirm, explicit-only never the empty-default fleet; the genlock build DEFAULTS genlock_fifo=true + latency 3 + certified coercion so the "pin" step is a CONFIRM not a WS write; the `_ndi_inputs_matching` sp-*_video prefix-floor sentinel in latency_pins_verify.py/latency-pins-baseline.json; NDI output `RESOLUME-SNV (cg-obs)` + #1185 :5961 pin; the supervisor deploy→relaunch→confirm→cg-chain-verify runbook; #1295) → `.claude/rules/resolume-cg-obs.md` (auto-loads on its `paths:`)
- EVENT-mode CONTRACT (#722) facet decisions must be None-tolerant + fail-closed, never crash on a per-item or whole-facet None (#1225) → `.claude/rules/event-assert-none-tolerance.md` (auto-loads on its `paths:`)
...
- The ONE shared OBS-box appliance baseline (issue 1357: imag's box-level provisioning moved verbatim into `scripts/lib/obs-box-baseline.sh` + `obs-box-kiosk.sh`, run by BOTH setup-imag.sh and setup-strih.sh with per-box facts as arguments -- openbox on plain Xorg, preempt=full, de-jitter, maxperf, power envelope, the PM QoS CPU idle wake-up latency bound (150 us holder unit, grader row `cstate` = unit + no deeper C-state entered in a 1 s sample); ONE grader `obs-box-baseline-verify.sh` for verify-imag/verify-strih; rtprio stays OFF; the strih-lx conversion runbook) → `.claude/rules/obs-box-baseline.md` (auto-loads on its `paths:`)
- strih-lx Linux provisioning (setup-strih.sh/verify-strih.sh + the `scripts/lib/strih-provision.sh` role lib; the parallel-run `STRIH-LX (...)` NDI-output namespacing + dantesync-CLIENT-not-master contract; the strih FULL-build CI variant `obs-genlock-linux-x86_64-strih` with its ENABLE_BROWSER=OFF/CEF follow-up; the MiniFuse-4→PipeWire fail-loud audio TODO; obs-fleet `strih-lx` row + deploy strih-lx arm; issue 1317; issue 1361: `--box <name>` + the per-box FACT file `scripts/strih-boxes/<box>.env` parsed by the ONE loader `scripts/lib/strih-box-facts.sh` — TODO_OWNER refuses naming each fact, fleet row pinned not computed, the strih-lx byte-identity golden) → `.claude/rules/strih-linux-provisioning.md` (auto-loads on its `paths:`)
- strih-lx in-place recording decode (`[8/8a]` `recording-verdict-on-strih-lx.sh`) at IDLE priority on the E-cores (`strih_lx_lowprio_prefix` pure encoding + the single-quoted remote-shell replica that resolves `/sys/devices/cpu_atom/cpus` ON the box; the issue-1354 post-run relock-storm root cause; binary scp + partial pull byte-identical; issue 1351) → `.claude/rules/recording-verdict-on-strih-lx.md` (auto-loads on its `paths:`)
- Reading the strih OBS log from a helper/watchdog (the ONE platform-resolved reader `scripts/lib/strih-log-read.sh` — plain ssh `tail`/`wc`/`tail -n +N` on the newest spaced `~/.config/obs-studio/logs/*.txt` on strih-lx, the verbatim -EncodedCommand PowerShell on a Windows strih; `timeout` inside sshpass; `UserKnownHostsFile=/dev/null`; the `strih` os token for the MV-fps pair; the multi-box dev1 watchdogs branch on `strih_log_os` and keep their Windows read byte-identical; rig-health-audit.py's python twin (`headtail` op) pinned byte-for-byte to the bash; never a new inline Windows-only read; issue 1360) → `.claude/rules/strih-log-read.md` (auto-loads on its `paths:`)
- E2E gate preconditions (DanteSync servo, bundle-state-server) → `.claude/rules/rig-standing-services.md` (auto-loads on its `paths:`)
- Never mutate the rig while a broadcast may be LIVE — the shared `stray_session_check_assert` rig-busy guard runs before EVERY recording-e2e.sh mutation (bkshading pause / [0/8] parity / [2/8] / [2b/8]), reuses obs_phase2.py rig-busy-check (never a duplicated loop), fail-opens only when no readable box is busy, names WHAT streams key-free; the `rig-busy-check` single-occurrence + `[N/8]`-in-guard-label anchor traps; #1271 → `.claude/rules/rig-mutation-broadcast-guard.md` (auto-loads on its `paths:`)
- Why strih+stream OBS auto-RECORD on stream go-live and orphan after StopStream (the recurring RIG_BUSY-blocks-E2E cause): NOT camera-box, NOT `RecordWhenStreaming`, NOT adv-ss — it is a Bitfocus Companion controller (10.77.9.205, strongly inferred not logged) running a PRE/PRODUCTION/POST scene state machine on stream OBS's program scene: `PRODUCTION` (scene "PRO" AND streaming) StartRecords BOTH boxes; the OFF (`POST` trigger → StopRecord both, button 24/1/2) is gated on cutting stream program to "POST" — a headless CI/dev stream StartStreams on "PRO" but StopStreams without ever cutting to POST, so the off never fires. camera-box's #657 stray-signature self-heal MITIGATES only while a run's preflight polls (no code change); the durable fix is owner-side Companion (a stream-STOP `PRODUCTION OFF` trigger). Full mechanism + config quotes + recommended fix + read-only re-inspect; #1274 → `.claude/rules/strih-autorecord-coupling.md` (auto-loads on its `paths:`)
- Rig lease read-only HTTP exposure on dev1 (:8890/rig-lease.json — the #830 same-machine premise is false for restreamer's stream-box runner; the restreamer#349 consumer contract, port choice, fail-open direction, PrivateTmp=no gotcha) → `.claude/rules/rig-lease-http.md` (auto-loads on its `paths:`)
- ANY new dev1 `--user` systemd unit's hardening directives (PrivateTmp/ProtectHome are currently live-verified INERT no-ops under dev1's unprivileged-userns kernel policy — declared intent only, never assume active isolation without re-checking) → `.claude/rules/dev1-systemd-user-unit-hardening.md` (auto-loads on its `paths:`)
...
- `OBS_FLEET` — the ONE declared managed-OBS-box list (`scripts/lib/obs-fleet.sh`, the camera-set.sh analogue): `obs_fleet_boxes <facet>` derives every watchdog's `BOXES=` default (byte-compatible env override); `obs_fleet_is_home` is the RESOLUME-SNV traveling-box gate (OBS-WS :4455 not dantesync :8898, issue 811); report-only in version-integrity/rig-health (issue 787 rate-exempt); the production strih is `strih-lx|10.77.9.202|linux-genlock|always` (Windows strih row REMOVED at M4), per-facet membership by premise + class-resolved Windows-only actions (bundle-state restart, obs-liveness recovery) — issue 1317; #1296 → `.claude/rules/obs-fleet-list.md` (auto-loads on its `paths:`)
- Editing `targets.md` (`git add` is hook-blocked as a sensitive file; commit it via the pathspec form) → `.claude/rules/targets-md-editing.md` (auto-loads on its `paths:`)
...
- Sender-bounce reverify painter-order proof + receiver-wedge escalation (the painter-dark-vs-strih-receiver-wedge `received=` discriminator; the headless strih-OBS restart = on strih-lx `systemctl --user restart strih-obs.service` over plain ssh (unit-installed guard, issue 1317 part 4), on a Windows strih kill+sentinel over ssh + AHK respawn, NEVER an ssh GUI launch, with the AHK-presence guard; READ_FAIL≠no-recv; the `--warm-settle` projector-independent re-check, #1093/#1096/#1098) → `.claude/rules/mv-reverify-escalate.md` (auto-loads on its `paths:`)
- `[4c/8]` frozen-camera gate leg-liveness = strih `genlock-fifo received=` counter DELTA per input, NOT pixel-hash (content-dependent, false-FROZEN on a held/static receiver frame in the deploy wave); pixel-hash kept report-only; the THIRD received=-tap consumer reusing mv-reverify + frozen-input-health; abort on PROVEN freeze across attempts (never false-abort on INCONCLUSIVE/READ_FAIL); the #797 window>cadence rule; #1233 → `.claude/rules/frozen-cam-received-gate.md` (auto-loads on its `paths:`)
...
- cambox off-box remote logging (#1311, Finding 1 step 2 — get kernel+journal messages off the box in real time so the NEXT half-dead-stick death is diagnosable; the #1309 on-STICK journal dies WITH the stick): netconsole (kernel printk over UDP, survives the fs dropping off the bus) + systemd-journal-upload (rich journal to a dev1 sink, no rsyslog reinstall per #762); single source of truth `scripts/lib/remote-logging.sh` wired into setup-device `[remote-logging]`/STEP16-pkg + create-usb base image + verify-device `(ak)`; dynamic configfs target with dev1 MAC resolved at boot; ro-root cursor redirect to /run; the dev1 receiver installer (`scripts/dev1-remote-log-install.sh`, imudp :514 + journal-remote :19532) is a SUPERVISOR step + the MGMT_DEAD correlation aid → `.claude/rules/cambox-remote-logging.md` (auto-loads on its `paths:`)
- Realtime isolation subsystem (merged RT-conditional affinity.rs capture-IRQ routing + verify (ac); owner REJECTED Ubuntu Pro 2026-08-20 → STAGED two-step FREE kernel path: STEP 1 = `linux-lowlatency-hwe-24.04` (config meta = preempt=full, the imag-nb precedent — on the cam boxes it also pulls a new HWE image so it is reboot-class + single-kernel purge), STEP 2 = custom PREEMPT_RT via CI only with measurement data; reboot-class supervisor-only via the rt-kernel-upgrade.sh dry-run planner + runbook; safe atomic order reboot-before-purge; per-box GRUB drift; the before/after 10-min emit-jitter+underrun journal measurement gates STEP 2; defect 2 = per-thread SCHED_FIFO code-fixed (drop process-wide CPUSchedulingPolicy in both unit copies + `affinity.rs set_current_thread_realtime` + verify (ah)); #899) → `.claude/rules/realtime-isolation.md` (auto-loads on its `paths:`)
- DanteSync clock-offset gate (#8 precondition; issue 1372: dantesync 1.9.0 is graded on its CLOCK DISCIPLINE — the ONE `clock_discipline_class` classifier (PTP_PHASE_LOCK / LEGACY_SLEW / LEGACY_NO_SLEW / UNKNOWN) + python twin pinned by one TSV of live-captured `/status` — and the date master on its own `date_step_bound_ms` (a 1.11.0 master, detected by its `date_correction_falling_behind` field, on its micro-corrections: `DANTESYNC_DATE_MICRO_BOUND_MS` 5 + margin, falling behind = OUT, `date_micro_paused` = PAUSED rc 4 which the [0/8] gate refuses; the JOURNAL path grades the master by the capability its own `/status` reports -- `journal_date_grade_from_step`, no readable `/status` = the step bound, named), never the bare phase_slew flag or the 2 ms UTC bound; HTTP-vs-journal grading paths, the #1021/#1022/#1041/#1055 master-slew false-DRIFT lineage + master-independent journal step-correlation rescue, the onset-drift recency anchor, `DANTESYNC_GATE_LINUX_{JOURNAL,HTTP}_<NAME>` fixture seams: journal env is a FILE PATH not content, distinct-`updated_ts` requirement) → `.claude/rules/dantesync-clock-offset-gate.md` (auto-loads on its `paths:`)
- dev1 dante-clock alert watchdog (#1307, PRODUCTION-CRITICAL class, umbrella #1308): headless-fleet clock-loss paging — reads EVERY dantesync node's `:8898/status` (the roster DERIVED from the issue-1372 dantesync fleet: every camera_resolve cam, the obs-fleet boxes, dev1, and the audio-VLAN PCs mbc/fohabl graded against the audio grandmaster; resolume home-gated) and pages on not-locked / foreign-gm / `ntp_step_storm` (dantesync's OWN 120/h boolean, never a re-hardcoded literal) / DNS `video-clock.lan` unresolvable / grandmaster IP MOVED between passes / **NO_DANTESYNC** (#1308: box UP via a TCP probe but `:8898` dead = dantesync crashed on a live box; box down/unprobed = SKIP defer #1001, never a false page); pure `dantesync_clock_decision.py` (#1199 mirror, grading mirrors clock-offset-guard.sh field semantics) + `dantesync-clock-alert-watchdog.sh` reusing obs-watchdog-decision.sh confirm + rig-grandmaster.sh + the shared watchdog-tcp-probe.sh; version != `DANTESYNC_VERSION_PIN` REPORTED in the card, never a page (#1308); TIME-BUCKETED `--dedup-key` via the shared `watchdog_notify_key` helper re-pings „dokolecka" while it persists (owner ruling — the production-critical class of watchdog-notify-dedup.md, allowlisted in the #1206 sweep), recovery log-only; UNREACHABLE+box-down=SKIP defers #1001; ships DISABLED (supervisor enables on dev1) → `.claude/rules/dantesync-clock-alert-watchdog.md` (auto-loads on its `paths:`)
- dev1 measurement-audio presence watchdog (#1310, PRODUCTION-CRITICAL class, umbrella #1308): between-run paging that the mbc measurement-audio chain (cam2 speaker QPSK marker → mic → mbc Ableton 10.77.7.232 → Dante → stream OBS `mbc`) reads DIGITAL SILENCE — the instrument the whole A/V-sync leg reads, silent-checked today only in-RUN by the #748 preflight so a dead chain is invisible until the next ~300 s E2E burns a cycle (release E2E 34764817477 `max_volume -91.0 dB`). Reads the `mbc` peak LEVEL off stream OBS via the `InputVolumeMeters` WS event (`measurement_audio_meter_probe.py`, NO recording/disk/mutation), classifies SILENT/PRESENT at the SAME -60 dB #748 bar SINGLE-SOURCED via `audio_preflight_default_threshold_db` (never retyped); pure `measurement_audio_decision.py` (#1199 mirror) + `measurement-audio-alert-watchdog.sh` reusing obs-watchdog-decision.sh confirm + obs-fleet.sh + rig-mode-state.sh; TEST-premise EVENT-gated like splitter-port #1290 (EVENT→SKIP, UNKNOWN→fail-safe proceed) yet production-critical (rig-mode gate ⊥ fault-criticality); TIME-BUCKETED `--dedup-key measurement-audio-stream-<bucket>` via `watchdog_notify_key` (allowlisted in the #1206 sweep), recovery log-only; SKIP=stream WS unreachable defers #1001/#732, UNKNOWN=mbc meter absent; DETECTION-ONLY (cure = unmute mic/Ableton/Dante), ships DISABLED (supervisor enables on dev1) → `.claude/rules/measurement-audio-watchdog.md` (auto-loads on its `paths:`)
...
- dantesync FLEET UPGRADE mechanism (canary-first per-OS-class, pin-not-latest, self-heal-not-blind-rollback, `.ps1`-file Windows path, `--ntp-master ""` single-node verify, #876; issue 1372: the same `.ps1` refreshes `dantesync-tray.exe` from the same pinned release -- fetched before any stop, swapped after the service, relaunched through a temporary `BUILTIN\Users` task, a tray failure = a named WARNING never a rollback) → `.claude/rules/dantesync-fleet-upgrade.md` (auto-loads on its `paths:`)
- The ONE declared dantesync fleet (issue 1372: `scripts/lib/dantesync-fleet.sh` + python twin `scripts/dantesync_fleet.py` — every camera via the camera_resolve walk, dev1, the obs-fleet boxes, the audio-VLAN PCs mbc + fohabl, lv1 excluded by the owner; per-node OS/role/home-gate/ssh user + a credential referenced by env-var NAME (fohabl, dev1-local 0600 cred file, parsed not sourced); the version gate + fleet upgrader `--fleet`, the clock watchdog defaults + `fixed` arm graded against the ROLE's grandmaster (audio 10.77.7.106), the handover item; the report-only per-role config drift check `scripts/dantesync-config-drift.sh` vs `scripts/dantesync-canonical-config.json`, BOM = drift, never writes a box; the E2E `[0/8]` gate deliberately stays per-run) → `.claude/rules/dantesync-fleet.md` (auto-loads on its `paths:`)
- Inter-PC VBAN rate + loss (issue 1372 part C: `scripts/vban_rate.py` pure pcap/pcapng parser — rate by the FRAME COUNTER least-squares slope, never the packet count (the −125 ppm mistake); loss = counter holes, dup/reorder never loss, jumps split segments; snaplen ≥ 96; `--dst` keeps inbound streams; the dev1 `vban-rate-alert-watchdog.sh` 60 s strih-lx tcpdump with the VBAN-magic BPF, provisional ±20 ppm / 1e-4 bounds, production-critical re-ping keyed on name + sender IP, report-only, ships DISABLED) → `.claude/rules/vban-rate.md` (auto-loads on its `paths:`)
- obs-vban send pacing (issue 1372: the vendored norihiro/obs-vban 0.3.1 on the cg OBS, the VBAN sender to FOH/lv1; the send thread keeps a jitter buffer (`pacing_target_ms` "Send Buffer", default 64, clamped 20-200) and sends packet n at t0 + n x packet_duration on the disciplined `os_gettime_ns`, every due packet per wake, underflow waits (no zero-fill) + overflow drops the oldest, both counted; t0 = first full packet + target, never "depth reached the target" (underflows the logged 55 ms stall); same packets + `nuFrame` as 0.3.1; pure `vendor/obs-vban/src/vban-pacing.h` <-> `src/vban_pacing.rs` + the compiled-header parity gate + the logged-pattern bench; built in both windows-genlock*.yml, staged into `obs-plugins/64bit`, FULL deploy backs it up + byte-verifies it via `scripts/lib/genlock-plugin-deploy.sh`; the `obs-vban pacing:` 10 s status line) -> `.claude/rules/obs-vban-pacing.md` (auto-loads on its `paths:`)
- RESOLUME-SNV dantesync maintenance (the traveling CG box's dead NTP phase while strih is off + phase_slew off: the PURE `scripts/dantesync_config_patch.py` clock-policy (1.9.0 `clock_discipline=ptp_phase_lock` default, `--legacy-phase-slew` for a pre-1.9.0 node)/ntp_server-parametrised patcher that NEVER touches ntp_server_mode + its EMIT-only PS apply program; `scripts/dantesync-maintenance-gate.sh` REPORT-ONLY tier reusing the clock-offset-guard parsers + `obs_fleet_is_home`, SKIP-when-away never a false red, never in `[0/8]`; resolume in the fleet-upgrade `--win` traveling path via `dantesync_resolume_win_spec`/`dantesync_skip_away_traveling`; the supervisor RUNBOOK + strih-OFF/ON acceptance; the ntp_server repoint target is deferred to dantesync#111; issue 1297) → `.claude/rules/resolume-dantesync.md` (auto-loads on its `paths:`)
...
- CG-path burn-id node role (SongPlayer 911014 chain-ORIGIN painted by songplayer#151 — never in `CAMERA_UNDER_TEST_NODES`; cg OBS 911015 HOP at `burn_geom::Corner::BottomCenterRight` via the `resolume` host-role; both tick-excluded + `all_burns`-excluded; the crate-root `src/cg_chain_gate.rs` per-hop contiguity+hold REPORT-ONLY seam that never touches the camera-chain `overall_pass`; the opt-in `CG_CHAIN=1` leak-guarded E2E profile whose cg recording is decoded IN PLACE on RESOLUME-SNV (`recording-verdict-on-resolume.sh` / `--extract-partial cg`, a bounded-grace background collect, one CG-LEG run-log line, issue 1302 job-budget slice); the GENERATED decode fixture a real cg-OBS frame must replace once songplayer#151 ships; #1301) → `.claude/rules/cg-burn-node-role.md` (auto-loads on its `paths:`)
...
- Leg-health `[0/8]` preflight capture-health signal (the HARD gate is FRAME LOSS = `max(0, sent-captured)` from the `Streaming:` lines — sent=emit-cadence, captured=device frames, over-rate benign #909; NEVER re-gate on the DEQUEUE STALL count — VIDIOC_DQBUF blocking-wait duration is ANTI-correlated with real loss, issue 1198; 1.25% calibrated gate + 3-window sustain guard; a dead leg is caught by emit-SKIP/EPROTO/#656 not this metric; #1133) → `.claude/rules/leg-health-frame-loss.md` (auto-loads on its `paths:`)
- Classifying a `#707 genlock emit-gate SKIPPED N boundary interval(s)` event as LOSSY vs BENIGN (the N is a boundary-COUNTER leap, NOT N lost frames; the `!queue_had_frame` resync is the honest clock-STEP path — capture keeps flowing 60fps, emit==cap; triage = SOURCE `#707 emit-1s/cap-1s` bucket deficit around the skip + RECORD `verdict.segments[]` window/883-fallback correlation; buffered-drain already catches up 1 not leaps; issue 1131 closed overcome 2026-09-01 — nearest-frame degrade NOT needed, re-open only on a sustained cap−emit≥2 or a non-883 multi-slot residual) → `.claude/rules/emit-gate-skip-triage.md` (auto-loads on its `paths:`)
- In-process USB self-heal action SEQUENCE — ONE helper `capture_rate_selfheal::attempt_self_heal(...)` that ALL FOUR triggers call (#656/#663/#971 capture-rate, #1128 grabber-STUCK, #1193 sustained OVER-RATE, #1200 LATCH-HALVING; add a 5th via a `SelfHealMessages` const, never a re-inlined copy), plus the #1201 shared outer pieces for the per-trigger-floored triggers: `cooldown_elapsed` (the one floor predicate) + `attempt_floored_self_heal` (env-gate → pending-is-none → floor → attempt, the wrapper both #1193/#1200 main.rs blocks now call); each trigger keys on a DIFFERENT capture signature (see the four-trigger discriminator-band table in the rule); emitted log lines are BYTE-anchored by dev1-watchdog grep patterns (harness_capture_rate_guard / harness_self_heal_attribution_895 test scripts/lib patterns vs hardcoded samples — neither reads main.rs), so preserve substrings; the real reset is injected for Tier-0 testability, #1149/#1193/#1200/#1201) → `.claude/rules/capture-selfheal-action-sequence.md` (auto-loads on its `paths:`)
...
- Reading the stream `asrc: source 'mbc' estimated=` residual (issue 1372: since dantesync 1.9.0 + the disciplined Windows media clock it reads ≈ 0 ppm, so a steady ≥ 3 ppm means a clock left the Dante tick — check dantesync `rate_source`/`f_phase` and the `media_clock` facet first, never tune the ASRC; the history: ≈ +8 ppm was the pre-1.9.0 Dante-GM-vs-UTC floor mirrored by `f_phase`, ≈ −18 ppm the DVS/PTP port collision fixed in dantesync 1.8.53, dantesync#109 / #1265) → `.claude/rules/asrc-residual-floor.md` (auto-loads on its `paths:`)
- Optical undecodable floor gate (`gates_overall_pass()` seam mirroring #914, strict-vs-relaxed independence, scoped JSON flag naming; #915 made it report-only, #905 item 3 RE-GATED it LIVE at floor 6 — the 60Hz baseline is permanent) → `.claude/rules/optical-undecodable-floor-report-only.md` (auto-loads on its `paths:`)
...
- Changing the cross-camera `SPREAD_THRESHOLD_MS` (the ONE constant shared by the blocking SOURCE-side + (since #1142) equally BLOCKING DELIVERY-side spread gates — a `delivery cross-camera spread … → FAIL (LIVE — folds into all_pass)` log line is a real red, not a report-only cross; its full lock-step consumer list incl. the easy-to-miss `src/lib.rs` mod-decl comments + the probe-gated `recording_verdict_merge_gate_exit_code.rs` subprocess test; the one-directional flip risk; #1120/#1121 re-tighten) → `.claude/rules/spread-threshold-consumers.md` (auto-loads on its `paths:`)
...
- lipsync-test-mode.sh playback is `mpv --vo=drm` NOT raw fbdev (#1187): the `--audio-delay` SIGN gotcha (negative delays video = the old positive video `-itsoffset`; flipping it doubles the offset), the fb0-blank reuses `rig_test_ledger_clean_paint_fallback_cmds`, the `LIPSYNC_MPV_BIN`/`LIPSYNC_DRM_DEVICE` seams, the `LIPSYNC_PLAYBACK_GAIN_DB` fixed-speech-gain seam (#1191, default 9, REMOTE-expanded literal `${…:-9}` — calibrated to the asset peak into the mic-chain AGC operating point, NOT a dynamic normalizer), `LIPSYNC_AUDIO_LEAD_MS` default now 0 (#1191, 408 was ffmpeg-era; 408 = a recalibration option), mpv provisioned STEP-16 + verify-device `(x2)`, live playback + recalibration are supervisor rig steps → `.claude/rules/lipsync-drm-playback.md` (auto-loads on its `paths:`)
...
- Projection-tap scanout-TEAR detector (issue 781 + the issue-1196 v2 aux Vernier tick pair — cam2's grabber captures imag-nb's HDMI, so cam2's sweep leg IS the projection path; tear = UNION span of the primary dual-QR + the bottom aux tick pair > 1, merge-side, no schema bump; `AUX_TICK_RUN_ID` 911013 in `NODE_BURN_RUN_IDS` + the python mirrors; **LIVE since #1196** — `gates_overall_pass()`=true, `TEAR_FRACTION_CEILING`=0.005, scoped to `viability=observed`; the OPERATIVE signal is the AUX SINGLE-MARK cross-band (primary band span is always ≤1 = blind; known-torn run 1700989544), so `aux_any_decode_fraction` is the honest operability metric while both-mark `aux_decode_fraction`≈0 on CAM2 — never drop the aux or the gate goes blind) → `.claude/rules/projection-tap-tear-detect.md` (auto-loads on its `paths:`)
- A MULTI-SOURCE cambox window (issue 1367: the camera films an OBS monitor, e.g. cam2 on the strih-lx multiview) is judged by its node burn — the tear detector's `multi_path_suspect_fraction` over `MULTI_PATH_SUSPECT_CEILING` (0.10, NOT the 0.005 tear-rate ceiling) makes its copies/gaps, the three cadence gates and its `frozen_leg` REPORT-ONLY while the node-burn contiguity + hold, presence, the optical floor, the tear gate and self-heal stay blocking; pure `src/multi_source_window.rs` + `segment_continuity_scoped`, tear stats computed before the continuity fold, the `multi-source (report-only by #1367 decision)` tag in the JSON and both Discord renderings, the real-window TSV fixture + the literal-safe rustc/clippy-driver replica → `.claude/rules/multi-source-window.md` (auto-loads on its `paths:`)
...
- NO analysis on libobs's video-output thread (issue 1367: the dock's `st_raw_video` shares that ONE thread with every NDI output + recording, so an inline QR decode made OBS skip 28 % of output frames on the cg OBS; `raw_video` only copies into the two-buffer latest-pending `camera-box-decode-mailbox.hpp` and a worker decodes with the frame's own timestamp; `decode_dropped=` appended to the diag line; start before data capture, join in stop/destroy/dtor before quirc_destroy; the thread-ownership table; the g++ selftest + TSAN-under-`setarch -R` + mutation recipe; the local `-fsyntax-only` of sync-test-output.cpp with two stub headers) → `.claude/rules/av-sync-dock-decode-worker.md` (auto-loads on its `paths:`)
...
- One canonical genlock deploy path across the whole rig from ONE anchor CI run id (`scripts/deploy-genlock-fleet.sh` planner: same-SHA cross-workflow resolution; Windows EMIT-only via win-* MCP, imag ssh; the strih-lx EXECUTE arm `scripts/lib/strih-lx-deploy.sh` (sweep stale stages via obs-backup-retention --local-sweep keeping the deploying one, stage BEFORE the graceful stop, `setup-strih.sh --box strih-lx` detached with the GH token on stdin only and never an OBS start over a running installer, fail-closed read-back of marker + installed libobs bytes + render tick + a MainPID/NRestarts held for a settle time + :8899; prepare/apply split so every box resolves before strih-lx changes, Windows programs printed after the verified strih; plan == execute builders; issue 1317 part 6); the AHK stop→restart-verified-BEFORE-launch contract (managed mode only — superseded for resolume by the issue-1372 guard mode, which never restarts AHK); imag ships the WHOLE bundle per issue 1026; the shared `genlock_write_markers` marker helper + print-only retention, #789) → `.claude/rules/genlock-fleet-deploy.md` (auto-loads on its `paths:`)
...
- "Is this address the Linux strih?" (the ONE alias-aware authority `obs_fleet_name_for_host` behind both the Windows-tool class gate and `strih_platform`, class-keyed — `strih.lan` still resolves to .202; python twin `scripts/obs_fleet_table.py`) + the retired Windows-strih remnant inventory (the #1199 NIC watcher REMOVED with the PC; issue 1317 part 4) → `.claude/rules/obs-fleet-list.md` (auto-loads on its `paths:`)
...
- Attributing a copies/gaps residual to SOURCE (grabber) vs DOWNSTREAM (genlock-FIFO/60→30 decimation) from the cambox burn logs (`scripts/residual_churn_attribution.py`: the `#889 late-dupe copies emitted`/`starvation last-frame repeats` + `#707 emit-1s/cap-1s` + `Streaming: sent−captured` signals; the discriminator is BASELINE-RELATIVE — a steady deficit or steady `corrupted` floor is background NOT a source anomaly; issue 1242 finding = the ~0.06% churn is DOWNSTREAM, per-box cadence is a covariate, 609 clean-run emit-fills → 0 residuals) → `.claude/rules/residual-churn-attribution.md` (auto-loads on its `paths:`)
...
- Windows OBS media clock runs at the dantesync-disciplined RATE (issue 1372 part A: `os_gettime_ns()` in `platform-windows.c` integrates QPC at the system-time rate `inc / adj` from GetSystemTimeAdjustmentPrecise -- a LARGER adj is SLOWER, measured 1:1 per second live; units are QPC counts; resolved at runtime from kernelbase.dll, kernel32 does not export it; follows the rate, never dantesync phase STEPS, and is not the Dante GM clock (`f_phase` remains vs a Dante-clocked peer); rebase only on a rate change, 250 ms poll, one writer, lock-free sequence-counter readers that sample QPC inside the validated window; 1000 ppm clamp, rate 1 without dantesync; `os_sleepto_ns` waits on the same clock; WASAPI raw-QPC and CEF TimeTicks stamps mapped by their age on their own clock via the header-only `util/windows/qpc-timestamp.h` (so a deploy needs the FULL bundle, not only the fast obs.dll); vlc-video is not built (ENABLE_VLC=OFF), so a stock VLC source drifts (known limit); the Tier-0 authority `src/os_clock_discipline.rs` + the scripted AND threaded fake-Win32 parity gates with injected stalls + the mutation proof; post-deploy the mbc ASRC `estimated` moves to ≈ −f_phase by design) → `.claude/rules/windows-disciplined-media-clock.md` (auto-loads on its `paths:`)
- A dantesync fleet DATE step on the OBS boxes (issue 1372: the wall clock steps ~50 ms every ~1.8 h while the media clock follows only the rate — ONE render-tick wall-step detector `obs-genlock-wall-step.h` ↔ `src/genlock_wall_step.rs` re-grids the tick in ONE tick instead of the 2 ms/tick slew-back (DistroAV floors wall at emit, so its stamps follow, no sender change), the LOCK indicator BOOKS a ≤ 66 ms step (`genlock_qpc_wall_step_rebase_ms`, one `genlock-wall-step:` line, a clock set or a step storm still DEGRADE), the ASRC pays a CONFIRMED sample loss back at 1000 ppm (`step_recover_ms`, setpoint moves with the buffer, NOT booked out of the TS-smoothing timeline, `recover_ms=` on the `asrc:` line); the STEP 0 finding that the `mbc` drop was a REAL upstream loss; the two-clock bench; the resolume `cg-obs` bursts are a video-io lag, not this) → `.claude/rules/genlock-wall-step.md` (auto-loads on its `paths:`)
- In-OBS GENLOCK LOCK indicator (the statusbar LOCKED/DEGRADED/UNLOCKED widget: the pure `genlock_decide_lock_state` decision Rust↔C parity pair + `src/genlock_lock_state.rs`; the shared `obs_genlock_stats`/`genlock_fill_stats` read by BOTH the audit log line and the API; the per-output wall-stamping API + DistroAV hook; the always-on 1 Hz `QTimer` + async dantesync `:8898` clock poll; the output facet is ABSENT-never-UNLOCKED on a pure receiver; the `genlock-lock:` log family + the 3-copy pwsh/guard lock-step; FULL-BUNDLE deploy; #1298; the issue-1372 part-D MEDIA-clock (audio clock) term: the time-weighted rate of the wall-vs-media offset the widget samples in µs, over 600 s (a pair > 100 µs off the median rate is a wall step and left out) > 2 ms or a Windows raw-QPC fallback via libobs `os_gettime_discipline()` DEGRADES `media_clock`, never UNLOCKS, JSON v7 `media_clock` object) → `.claude/rules/genlock-lock-indicator.md` (auto-loads on its `paths:`)
- Fleet-visible genlock LOCK facet + dev1 watchdog (the CONSUMER of #1298: the widget emits its decided verdict on a versioned `genlock-lock-json:` heartbeat line — the log-line transport CHOSEN over a WS request because the clock facet is widget-private; `bundle_state_gather.genlock_lock_facet_from_log` → the nested `genlock_lock` facet attached AFTER build_bundle_state, omit-when-absent never a false UNLOCKED; `obs-fleet.sh` genlock-lock facet = strih/stream/imag/resolume; pure `genlock_lock_decision.py` + `genlock-lock-alert-watchdog.sh` page UNLOCKED/DEGRADED after 2-pass confirm, stable dedup `genlock-lock-$box`, resolume home-gated, SHIPS DISABLED; the `genlock-lock-json:` vendored string's 3-copy lock-step + mutual-non-substring vs `genlock-lock:`; the `GENLOCK_LOCK_FETCH_CMD` dry-run seam; rig-status chip via a report-only feeder token; #1299) → `.claude/rules/genlock-lock-facet.md` (auto-loads on its `paths:`)
...
- OBS projector window path = the STOCK upstream toplevel on every OS (issue 1357 RETIRED the issue-1352 Linux child-host + the `strih-mv-host` re-hosting helper: an XWayland+NVIDIA-PRIME present-stall workaround no box needs on the plain Xorg NVIDIA-primary baseline, and the hosted shape blacked the projector on a runtime always-on-top toggle because `SetAlwaysOnTop`'s `setWindowFlags`+`show` recreated the host's native window; never re-add a per-OS projector host, fix the display stack in the shared baseline; the `tests/obs_projector_stock_toplevel_1357.rs` guard; setup-strih step 8b removes a leftover helper, verify item 22 grades it absent; FRONTEND=full-bundle deploy; #1358 resize debounce) → `.claude/rules/obs-projector-stock-toplevel.md` (auto-loads on its `paths:`)
- imag HDMI Program OUT of Xorg → in-OBS vendored DRM-lease output (the forked OBS leases the HDMI connector via `xcb_randr_create_lease` and page-flips directly, no NDI hop/no presenter; module in libobs NOT a plugin — linux-genlock.yml is `ENABLE_PLUGINS=OFF`; X RandR name `HDMI-1` ≠ DRM connector `HDMI-A-1`; DEFAULT-OFF `~/.camera-box/drm-output.json`; the fsyntax-vs-real-headers net; M1=lease+solid-flip, M2=Program dma-buf scanout (both rig-verified), M4=lease-tolerant wrapper/seeder + drm_output facet + the ENABLE-flip/ORDER-MATTERS-rollback runbook; #1152; the selectable VIEW Program / built-in Multiview on strih-lx: the frontend-registered Multiview rendered into an sRGB texrender then raw-blitted like the Program, budget-gated by `obs_aux_sender_should_skip`, the OBS Tools switch persisted to `"view"`, a multiview view on imag = drm_output DRIFT, issue 1346; the second backend vk-direct = NVIDIA Vulkan direct display for the strih-lx BUILT-IN HDMI-0 (the NVIDIA X driver refuses the lease): `"backend":"vk-direct"` from the box fact `STRIH_HDMI_OUTPUT_BACKEND`, headers-only Vulkan + a dlopen loader, GL memory-object + semaphore interop, the output must have no X CRTC (-13 otherwise), the on-box rig harness `tests/c/drm_output_vk_rig_harness.c`, issue 1346) → `.claude/rules/obs-drm-output.md` (auto-loads on its `paths:`)
- DRM-latency measurement tooling (#1152 M3: the cam2-optical-differential capture+decode; the ffmpeg `-use_wallclock_as_timestamps 1` REQUIRES `-copyts` or the epoch rebases to ~0 = garbage; `-fps_mode` is an OUTPUT opt after `-i`; PLAN-default pure-builder orchestrator that NEVER writes drm-output.json; DORMANT−ENABLED delta cancels the grabber offset; reuse parse_payload/tick_map + qr_screenshot_check decoder) → `.claude/rules/drm-latency-measurement.md` (auto-loads on its `paths:`)
...
- imag NDI name-healing — the #1218 idle policy was REMOVED (#1230, owner ruling 2026-08-30 „ziadne nechcem mat hlupe idle policy"; all seven cameras always NAMED + alive, no receiver-sleeping); the KEPT #1158 healing via the ONE `enforce_ndi_names(obs)` point → shared #795-safe `obs_phase2.reenforce_ndi_name` (genlock_fifo True restored only on HEALED, OFFLINE untouched per #70); on-box lazy-`obs_phase2`-import safety, the `verify_parity` grep-qxF whole-line rule (no more idle line), passwordless imag WS → `.claude/rules/imag-ndi-active-set-policy.md` (auto-loads on its `paths:`)
- NDI discovery = a RECEIVER-side list of every managed sender IP + ONE output per cambox (issue 1342: the unconsumed `CAMn (30p)` stream removed, setup-device deletes its drop-in; `ndi-config.v1.json` `networks.ips` = the SDK finder's `p_extra_ips` registry, queried IN ADDITION to mDNS and never read by senders, so senders/stock TVs/guest laptops/the avahi port-map audit are untouched; GENERATED by the ONE pure `ndi_discovery_sender_ips` in `scripts/lib/ndi-discovery.sh` from `camera-set.sh` (every `camera_resolve` arm) + the obs-fleet `ndi-sender` facet (traveling resolume resolved at write time, never required), never hand-typed; written UNGATED by setup-device STEP 7 (/etc/ndi + the camera-box `NDI_CONFIG_DIR` drop-in, ProtectHome hides /root/.ndi) and setup-strih step 4b, graded by verify-device `(an)` / verify-strih item 34 against the SAME generator's pinned list (a renumber FAILs until re-provisioned); NEVER a `networks.discovery` (a configured sender stops mDNS — the part-1 Discovery Server model + its gates were removed); the BOM-free merging Windows `.ps1` + checked-in JSON pinned to `--json pinned`) → `.claude/rules/ndi-discovery.md` (auto-loads on its `paths:`)
- NDI SENDER port-map stability watchdog (dev1 baseline alert on a moved sender port that hands stock TVs the wrong source; avahi `-p` DECIMAL `\DDD` escapes + resolved-line field layout; the TWO NDI instances at one IP isolated by the anchor's mDNS-hostname group; baseline stores name→port only so hostname-suffix drift never matters; empty/anchor-absent map = gather error never a page; the DistroAV FINISHED_LOADING deferral root cause for the #1185 PGM-pin; python-shells-to-bash Tier-0; #1181; the watched strih RESOLVED from the obs-fleet `ndi-portmap` facet + anchor-record IPv4 IP + exit 4 = baseline captured on another strih, issue 1363) → `.claude/rules/ndi-portmap-watchdog.md` (auto-loads on its `paths:`)
- DistroAV SENDER-output lifecycle + NDI port ordering (ports assigned by CREATION ORDER from :5961; the program starts LAST at FINISHED_LOADING so ndi_filter republishes win the low ports; #1185 reserve-at-obs_module_post_load + adopt-in-ndi_output_start pins 2ME PGM to :5961; the begin_data_capture-fail leak that turns the pin-holder into a frameless PGM ghost; call-site-unique lock-step anchors; the frameless-idle-sender tolerance is UNVERIFIED without the rig; Linux: libndi binds senders without SO_REUSEADDR so a TIME_WAIT on :5961 from the previous OBS walks the pin to :5962 — FIXED by the Linux-only `ndi-sender-port.{h,cpp}`: every sender create tracked (port = the one new /proc/self/fd listener in the sender band [5961,6960) -- libndi RECEIVERS pulling a remote source open their own :6960+ listeners concurrently; unidentified = PORTID-1363), every sender destroy preceded by SO_LINGER {1,0} PLUS an immediate AF_UNSPEC reset (libndi shutdown()s before close(), so linger alone leaves TIME_WAITs), WARN-1363 only at the reserve, never a wait; issue 1363) → `.claude/rules/distroav-sender-output-lifecycle.md` (auto-loads on its `paths:`)
...
- Receiver-side AUDIO genlock parity (#1303 — the audio leg made a first-class genlocked signal: the audio placed at `source_output_audio_data` at its NDI timecode + the video's MEASURED stamp→present delay (the presented frame's age at the scheduled tick, EMA + half-frame hysteresis + settle-then-apply) through the LIVE wall→mono offset — a shallow source's audio follows its LATCHED per-lock depth, is WITHHELD until a delay is known (≤ 10 s, then the #1303 latency hold), and every hold change while playing is SLEWED through the ASRC resampler at 1000 ppm (steps booked out of the TS-smoothing timeline + into the level setpoint), never a step re-placement (issue 1367, ROZHODNUTÉ 5827497952); a >2 s timestamp jump's `handle_ts_jump` reset must PLACE an active hold, never append at the arrival instant (live 25.9.2026 12:31), and `audio_pairing_offset_ms` reads the MEASURED placement of the samples (`audio_place_err_ms=`), plus the `audio-stall #1367:` mixer-tick probe in obs-audio.c, orthogonal to the ASRC rate servo; the pure `src/genlock_audio_pairing.rs` decision + its contiguous static-inline C mirror block + parity gate + the two-clock bench; the audio facet in `obs_genlock_stats` v2 + the `genlock-fifo audit` line + jitter_audit; the cg-chain audio column; the 3-copy lock-step + the Tier-0 recipes; deferred followups = the LOCK-indicator audio term + the per-box certified-table audit) → `.claude/rules/genlock-audio-pairing.md` (auto-loads on its `paths:`)
- Genlocked NDI SENDER contract — the normative RFC-2119 spec ANY external sender (SongPlayer, the `cg` OBS, future senders) must meet to lock to the fleet clock: FLOOR-boundary 100ns-epoch `timecode` (never ceil/synthesize/0/monotonic), `clock_video=clock_audio=false`, epoch-grid pacing (catch-up ≤8 / resync / re-latch / starvation-repeat), audio raw-wall-clock timecode at real-time rate, 1Hz lock-state observability, acceptance = receiver `genlock-fifo audit` ground truth (±20ms skew, zero pathology deltas ≥1h). The camera-box `src/` + vendored OBS/DistroAV are the cited reference implementation (issue 1294) → `docs/genlock-sender-contract.md`; its keep-in-sync pointer → `.claude/rules/genlock-sender-contract.md` (auto-loads on its `paths:`)
- genlock-FIFO settle-wait (`scripts/lib/genlock-settle.sh` — the `[4j/8settle]` step between `[4i/8align]` pin writes and `[5/8] StartRecord`: wait on the genlock-fifo audit relock/underrun/late_hold DELTAS going quiet (dropped_due is parsed + watched for reset but EXCLUDED from quiet — strih inputs decimate 60fps to 30fps so it structurally advances every tick, issue 1221 follow-up), NOT a blind sleep; raw counter-value deltas so #797-immune; TWO independent bounds — wall budget + pass ceiling — fail-open never abort; ALWAYS-exit-0 under the caller's `set -euo pipefail` (#1133 class); injectable reader/clock/sleep seams for Tier-0; issue 1221) → `.claude/rules/genlock-settle-wait.md` (auto-loads on its `paths:`)
- ONE per-second genlock grid (#1355: the receiver ts-align deadline + render tick AND the camera emit gate (step 3: `genlock_advance_boundary` / grid slots, never `+ interval` / `/ interval`) now floor on the SAME per-second grid the senders stamp on — the 1970 grid lost 10 ns/s and walked the 2ME PGM FIFO 31↔32 with the date; shared header `obs-genlock-grid.h` ↔ `src/genlock_grid.rs`, integer rates only, day-long C↔Rust parity + sender-coincidence gate; the `src/genlock_grid_bench.rs` N==1 bench that reproduces the live flip on the 1970 grid and 0 drains on the production grid; the arrival-side `stamp_dup=` / `stamp_gap=` audit counters; what changes on deploy + the ≥30 min post-deploy audit) → `.claude/rules/genlock-per-second-grid.md` (auto-loads on its `paths:`)
- N==1 PIN-DERIVED DEPTH (issue 1367: a deep N==1 input like the stream `NDI 2ME PGM` settles on `base + 1` frames after EVERY restart — `base = ceil((pin − 1 µs)/interval)`; shed/hold one frame on the ROUNDED presented age read at the render tick's SCHEDULED instant (`video_time`, never the processing wall — `video_sleep` catches late ticks up), deep sources only, and only while that scheduled tick is within 2 ms of the grid (a wall-clock step slews back 2 ms/tick); `genlock_phase_converge_due` untouched, the SOURCE wrapper routes N==1; `n1_grows=` audit field; the early-phase and queue-length-drain limits; the Tier-0 + mutation recipe; a SHALLOW N==1 source (cg feed at 3 ms) latches `max(base, rounded arrival floor) + 1` per LOCK (ACQUIRE / ≥ 1 s GAP RESYNC / pin change) over 90 on-grid present ticks and is held there by the same hold/shed with the #859 drain kept out, the imag min-latency marker `~/.camera-box/genlock-min-latency` caps it at base + 1 and reports it, the `genlock-shallow-lock` line + `shallow_depth=` tokens, the `genlock_shallow_av_bench.rs` two-clock bench, ROZHODNUTÉ 5827497952; design 5830750134: the latch reads the window's p90 not its max, a >1-frame p10–p90 spread re-measures (bounded), D is clamped to base + 3 and reported, an unreachable D / a backlog relock storm re-measures, the shed never chases a D the arrival cannot supply, the audio hold follows the realized delay under the same lock, `genlock-shallow-remeasure` reason lines; design 5833339163: a GAP RESYNC HOLDS a governed shallow source until the post-gap head is D old — a skipped sender stamp (song change) never puts the video under D, unless the head is already duplicated behind it (the late-label residual); ROZHODNUTÉ 5842640404: the latch histogram reads the RECEIVE-time arrival lag + the 15 ms jitter budget (`genlock_rx_arrival_lag_ns`), the rise/fell watch keeps the raw tick floor; ROZHODNUTÉ 5842848307: no budget on the min-latency (imag) box; design 5844353368: a per-source STICKY content floor (the p90 budgeted latch floor of a 90-tick block observed while the audio hold mode is timecode, spread/base/over-clamp-guarded, decays one frame per 30 min, in-process, none on the imag marker) — a lock latches `max(p90, sticky) + 1`, so an idle SongPlayer re-lock keeps the song depth and later song starts never re-measure or slew the audio) → `.claude/rules/genlock-n1-pin-derived-depth.md` (auto-loads on its `paths:`)
- N>=2 conveyor arrival-jitter BUDGET (`GENLOCK_N2_JITTER_BUDGET_NS` = 15 ms, single-sourced across `should_converge_phase` in `src/genlock_backlog.rs` + the vendored C `genlock_phase_converge_due` + the probe delegator; `budget = max(hysteresis, jitter budget)`; the parity gate MUST `lift_define` the new `#define`; the FINDING that SimConveyor1049's re-anchoring follower CANNOT reproduce the live phase ladder so the DIRECT arithmetic threshold test is the faithful RED→GREEN proof; the scope-3 `genlock_audit_snapshot.py` holds/relocks/converge_sheds audit-delta recipe that NAMES the ladder victim, report-only in `compose_report`; #1354) → `.claude/rules/genlock-conveyor-jitter-budget.md` (auto-loads on its `paths:`)
...
- E2E per-run Discord report — TWO renderings (`compose_summary` short/`--json-chunks`/Discord vs `compose_report` full-detail/plain/CI-log); the caller captures stdout `2>&1`+jq so `--json-chunks` must print ONLY the JSON array; blocking-vs-report-only is DERIVED from each verdict-JSON seam's `gates_overall_pass` mirroring recording-verdict.rs's `all_pass` fold (SOURCE spread AND — since #1142 — DELIVERY spread BOTH block; report-only NEVER `❌`; PASS=3 lines); #711/#1127 → `.claude/rules/e2e-discord-report.md` (auto-loads on its `paths:`)
- Pixel proof for merge-classified slots with `png: null` (issue 1367: `scripts/lib/missing-slot-pixels.sh` after the merge, before the report + #652 cleanup plan; stage 1 = the verdict's own CFR rawvideo decode, stage 2 selects by raw-frame ordinal with `-framerate 1 -frame_pts 1` so the PNG name IS the frame_index — never `select` on the source file; camN -> strih recording, strih/stream -> stream recording; strih-lx at the on-box idle priority, stream through a `.cmd` because PS 5.1 pipes are not binary-safe; cap 12 slots; report-only `missing_slot_pixels` verdict block) → `.claude/rules/missing-slot-pixels.md` (auto-loads on its `paths:`)
- E2E recordings retention — dry-run-first sweep of the OBS record dir (strih `C:\_REC` since 17.9.2026 — the D: NVMe failed, owner left recordings on C:, issue 1338; historically `D:\_REC`, ~691 GiB of old runs vs the 50 GB budget): pure `src/recordings_retention.rs` decision (keep newest-N runs UNION younger-than-D-days; EXPLICIT OBS-timestamp allowlist, NEVER a generic `*.mkv` sweep — `strih700105.mkv` stays PROTECTED) mirrored by `scripts/strih-recordings-retention.ps1` (scp -O + `powershell -File`, DRY-RUN default, `-Execute` = supervisor's reviewed step) AND, for the Linux strih-lx `/srv/_REC`, by the bash `--local-sweep` in `strih-recordings-retention.sh --box strih-lx` (record dir from the OBS profile, parity-pinned to the Rust plan() by ONE shared `tests/fixtures/recordings_retention_parity.tsv`, issue 1317); #1122 → `.claude/rules/recordings-retention.md` (auto-loads on its `paths:`)
...
- Switch-in transient classifier (issue 1144, REPORT-ONLY): `src/switch_in_transient.rs` recognizes an imag NDI-receiver spin-up transient (a dense leading burn-loss burst that recovers after a program cut) so the imag per-segment sweep ATTRIBUTES it to cold-cut instead of failing the content gate — FAIL-CLOSED (never excuse a few-drop/half-window-dead/mid-window/sparse/non-recovering/frozen/independent-stuck shape); folds ONLY through the report-only content seam; n=1 calibrated → the constants MUST be re-validated against the item-2 sick-camera run before the flip, never widened so a red passes; Tier-0 via a rustc replica (+ the focused-replica pattern for a small add to a big module) → `.claude/rules/switch-in-transient.md` (auto-loads on its `paths:`)
...
- OBS audio-timeline-lag alert (dev1-side watchdog: reads the `audio_ts_lag_ms` facet bundle_state_gather exposes on `:8899/bundle-state.json` — the MAX per-source `audio-telemetry #800 '<src>': ts_lag_ms=N` from obs-audio.c:698, parsed from the TAIL of the #1222 bounded read never a 2nd log scan, `-1`=no-timeline excluded, omit-when-empty; DETECTION-ONLY no auto-action since the cure is a live-box reboot owner-call; SKIP=fetch-fail defers #732/#1001, UNKNOWN=facet-absent, LAGGING>5000ms pages after 2-pass confirm; NEVER a false page since paging needs a fetched positive reading so no reference-anchor needed; pure `audio_lag_decision.py` #1199-mirror + `audio-lag-alert-watchdog.sh` reusing obs-watchdog-decision.sh; #1206 stable dedup-key `audio-lag-$box` + recovery log-only; ships DISABLED, #1226; #1231 adds the FRESHNESS `audio_ts_lag_age_s` facet + a STALE verdict — in-log recency anchored to the LAST line in FILE ORDER never max-seconds-of-day, telemetry stopped-while-log-advancing, never a false page) → `.claude/rules/audio-lag-watchdog.md` (auto-loads on its `paths:`)
- Render-freeze / relock-storm alert (dev1-side watchdog, issue 1320 guardrail for the cured strih PROGRAM render freeze): two arms off `:8899` — RENDER on `program_render_lagged` (a `lagged>0` window = a render-thread freeze; pages on `lagged >= a magnitude FLOOR` (default 30, above the relaunch band 1/2/11, below the 61/228 real freeze) AND a fresh age, so a relaunch startup-lag never false-pages) + RELOCK on `relock_bursts` (issue 1318's `summarize_relock_bursts` PORTED to the gather as `relock_bursts_from_log`, parity-locked to the Rust fixtures; ≥8 relocks/1 s = a storm); `render-freeze` fleet facet = strih stream resolume strih-lx; pure `render_freeze_decision.py` + `render-freeze-alert-watchdog.sh` reusing obs-watchdog-decision.sh; PRODUCTION-CRITICAL time-bucketed dedup (`render-freeze-$box` / `relock-storm-$box`), SKIP=unfetchable defers issue 732/1001, UNKNOWN=facet absent, DETECTION-ONLY recovery log-only, ships DISABLED, #1320 → `.claude/rules/render-freeze-watchdog.md` (auto-loads on its `paths:`)
- Upstream-audio-latency STEP detector (dev1-side watchdog, the #1265 A/V-residual early warning distinct from the #1226 ts_lag HEALTH axis: reads the av-sync dock's monitor-only `LOCK-CORRECT SUGGESTED genlock_latency_ms_src <pin> -> <new>ms (measured offset=<X>ms)` line — pin + offset INLINE, stream-only — summarized to `av_offset_*` facets; a rolling recent-vs-baseline median STEP at a CONSTANT pin pages report-only after 2-pass confirm; the pin is a covariate NEVER subtracted — `offset-pin` was FALSIFIED live so a pin move → REPIN; the rolling baseline self-normalizes a persistent step so RECOVERY is judged vs a FROZEN pre-step baseline; SKIP/STALE/UNKNOWN/REPIN never page; `av_step_decision.py` + `av-step-alert-watchdog.sh` reusing obs-watchdog-decision.sh, dedup-key `av-step-$box`, ships DISABLED, #1267) → `.claude/rules/av-step-upstream-detector.md` (auto-loads on its `paths:`)
...
- Gating a TEST-premise dev1 watchdog on rig EVENT mode (the splitter-port #739 family paged the owner 5× during a LIVE show — a camera-less cambox is legitimately black in EVENT where each cambox has its OWN camera, so the sibling-anchor DEAD_PORT premise is false; the EVENT signal is the DURABLE cam2 painter state = pidfile OR `cam2-painter.service` enabled via `scripts/lib/rig-mode-state.sh`'s 3-state EVENT/TEST/UNKNOWN classifier reusing optical-chain-health.sh, NOT the ledger and NOT the #281 heartbeat; fail-safe UNKNOWN→page-as-today; the per-watchdog audit — only splitter-port needed the gate, optical-chain already has it, grabber-stuck/cadence/frozen-input are mode-independent; #1290) → `.claude/rules/rig-mode-event-gate.md` (auto-loads on its `paths:`)
- "development" handover check — ONE dev1 command run when the owner hands the rig back after a production: `scripts/rig-dev-handover-check.sh` runs each EXISTING read-only probe (alert-watchdog `--dry-run`s, obs_burn_filter sweep-check, set-ndi-mapping --verify-only, latency_pins_verify, dantesync/camera-box version gates, rig-mode-state) bounded + drain-safe, and the pure `scripts/rig_dev_handover_decision.py` maps each item (mic/mode/painter/burns/mapping/pins/clock/obs/net/audiolag/genlock/dantesync/cambox) to OK/FORGOT-BY-OWNER/UNKNOWN + a Slovak line ending `zabudol si: …`; report-only never mutates, a DOWN box (cam2) fails safe to UNKNOWN never a false OK, absent genlock facet = UNKNOWN-not-forgot, scenes/Studio-Mode covered transitively by the `mode` item; #1312 → `.claude/rules/rig-dev-handover-check.md` (auto-loads on its `paths:`)
...
- NIC swap aftermath (a replaced network card leaves dantesync on a dead pcap handle — `ERROR_DEVICE_REMOVED` spam, no re-select, silent fall back to userspace NTP + step storm, gate reads `NTP STORM`/`PTP DEGRADED` while the grandmaster is reachable; cure = restart the dantesync service and verify the fresh `Found device:` names the NEW adapter; the `pcap_ntp_active:false` and Npcap-binding red herrings; the same swap also wedges strih NDI receivers into one frozen frame that only an OBS restart clears) → `.claude/rules/nic-swap-timesync-recovery.md` (auto-loads on its `paths:`)
...
- Per-camera NDI SEND stagger (`src/send_stagger.rs`: camN hands its frame to the NDI SDK (N-1)×1.2 ms after the emit gate so the seven per-tick bursts stop overflowing the strih-lx 2.5 GbE `foh1_video ether2` port; N from the OS hostname CAM<N> via one pure `plan()`, clamped to 45 % of the SHORTER of the send/capture interval; timecode + emit grid untouched; the sleep is added back into the buffered-queue dequeue signal via `idle_wait_ms` (taken BEFORE the dequeue); a backlogged frame skips the sleep; the per-5 s `#1242 send stagger:` budget line; the E2E burn path proves send timing but NOT the production capture-loop budget; the 1 GbE wire-time vs 2.5 G drain arithmetic + the live acceptance steps; issue 1242) → `.claude/rules/ndi-send-stagger.md` (auto-loads on its `paths:`)
- strih bandwidth ROLES — full-bandwidth NDI only for SHOWN cameras (owner ruling 24.9.2026, issue 1242): the program-path main `NDI camN` (`genlock_connect_on_show`, set ONLY by the strih scene role lib on fleet camera senders — stream/resolume/cg keep the issue-764 keep-alive) PARKS in the vendored receiver thread while hidden (receiver to the detached reaper, `genlock-park '<src>': state=parked` 5 s heartbeat; never the stock STOP_RESUME graphics-thread join) and reconnects on show; the always-connected low-bandwidth `MV NDI camN` #501 twins feed the built-in multiview via `MV <scene>` twin scenes (`scripts/strih_bandwidth_roles.py` via `strih_scenes.py --apply-roles` on every launch; nested scenes twinned too, swapped items bounds-pinned, membership handed off once (operator wins); NDI-output scenes Grading/Interkom stay shown; the custom MULTIVIEW grid swapped; scratch-scene multiview refresh); the vendored frontend `Multiview.cpp` resolves a twin cell to its `camera_box_multiview_target` program scene for the tally border, label and click; every received= consumer reads a parked input as hidden-by-design via the ONE parser `scripts/genlock_park.py` + bash twin `scripts/lib/genlock-park.sh`; the E2E HOLDS connect-on-show off (`scripts/lib/connect-on-show-hold.sh`, guarded, restored in cleanup) → `.claude/rules/strih-bandwidth-roles.md` (auto-loads on its `paths:`)
...
- Byte-safe extraction from a PS-fetched OBS-log tail (grep's binary-detection trigger is narrower than "any invalid byte in the stream" — co-residence on the SAME line matters; `LC_ALL=C grep -a` fixes only the shell side, a Rust `read_to_string` consumer downstream still needs the byte STRIPPED; pin `LC_ALL=C.UTF-8` in any byte-safety test harness or its RED silently stops discriminating; issue 1258/1262) → `.claude/rules/ps-log-byte-safety-extraction.md` (auto-loads on its `paths:`)
...
- NDI per-connection rate-halving auto-heal (dev1-side watchdog + the FIRST cure arm in the family: the `recv-timing #797 '<input>': n=… cap_avg=…ms` tap is PER-INTERVAL so the rate is measured WITHIN-PASS from the last two lines' OWN timestamps — never a wall-clock divisor, never cross-pass; HALVED = fps≤0.6×exp OR cap_avg≥2×interval, BORDERLINE holds confirm, HEALTHY resets; pure decision in PYTHON (`ndi_halving_decision.py`, pytest-Tier-0 per #1199) + bash orchestrator reusing obs-watchdog-decision confirm/throttle; no-double-page on BOTH receiver+sender #1001 state; healthy-sibling = context-only box-wide-vs-per-connection discriminator, never a page gate; CURE = obs_phase2 idle-receiver→restore gated OFF by NDI_HALVING_SELFHEAL + per-input cooldown, empty-name-window hazard per ndi-name-recovery; ships DISABLED, #1203) → `.claude/rules/ndi-halving-watchdog.md` (auto-loads on its `paths:`)
- VB-Matrix presence facet + dev1 alert watchdog (the `vb_matrix_running` bundle-state facet is 3-STATE — install-present disk gate + native tasklist parse; a FAILED tasklist read must be None→UNKNOWN never a false "0" DOWN, install-absent imag omits it; `wmic` is GONE on the stream box so start-time = CIM `Get-CimInstance Win32_Process` `.ToString("s")` locale-stable, #1222 PID-keyed-cached + called unconditionally; ONE shared `tasklist_csv()` feeds both obs-count + vb-matrix; the 5th obs-watchdog sibling `vb-matrix-alert-watchdog.sh` DETECTION-ONLY, cure = owner `schtasks /run /tn StartVBMatrix` + durable AtLogon-trigger, dedup `vb-matrix-$box`, ships DISABLED; worktree Tier-0 = the `VB_MATRIX_FETCH_CMD` env fetch-seam dry-run, #1227) → `.claude/rules/vb-matrix-watchdog.md` (auto-loads on its `paths:`)
- Alert-watchdog Discord notify discipline (EVERY `notify --body` = a phone ping: ALERT class gets a STABLE per-incident `--dedup-key` — box/source/verdict/leg-scoped, no per-pass component — so a repeated identical state EDITS the airuleset card instead of re-pinging; RECOVERY/status ✅ pings go to the machine channel (`log` line) NOT the phone; 🚨/⚠️/🛟/🧹=keyed-ping, ✅=log-only; delivery-layer only, detection untouched; enforced by `tests/python/test_notify_dedup_key_sweep_1206.py`; airuleset #704/#705, #1206. **PRODUCTION-CRITICAL class (#1308):** the 11 watchdogs whose fault production cannot run without TIME-BUCKET their key via the ONE shared helper `watchdog_notify_key` (bash, in obs-watchdog-decision.sh) / `scripts/watchdog_reping.py` (python twin) → re-ping „dokolecka" while it persists; the sweep allowlist is CLASS-based + rejects a bucketed key in any diagnostic/TEST-mode watchdog) → `.claude/rules/watchdog-notify-dedup.md` (auto-loads on its `paths:`)
...
- Per-box arrival-floor STAGE decomposition mining tool (issue 1168 task 1 — `scripts/arrival_floor_decompose.py`: which box/stage owns the cross-camera presented-age offset; `floor = latency_ms + mean_head_skew_ms` so `excess = Δlatency (strih-config) + Δskew (grabber/transport)`; uniform recv-timing cap_avg falsifies transport ⇒ grabber-owned; REUSE `arrival_floors_from_jitter`/`parse_recv_timing`/the jitter JSON, never a new skew regex; pure-Python Tier-0 RED→GREEN; supervisor mining only, no gate) → `.claude/rules/arrival-floor-decompose.md` (auto-loads on its `paths:`)
