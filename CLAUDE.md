...
- strih-lx Linux provisioning (setup-strih.sh/verify-strih.sh + the `scripts/lib/strih-provision.sh` role lib; the parallel-run `STRIH-LX (...)` NDI-output namespacing + dantesync-CLIENT-not-master contract; the strih FULL-build CI variant `obs-genlock-linux-x86_64-strih` with its ENABLE_BROWSER=OFF/CEF follow-up; the MiniFuse-4→PipeWire fail-loud audio TODO; obs-fleet `strih-lx` row + deploy strih-lx arm; issue 1317) → `.claude/rules/strih-linux-provisioning.md` (auto-loads on its `paths:`)
