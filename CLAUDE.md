...
- genlock-FIFO settle-wait (`scripts/lib/genlock-settle.sh` — the `[4j/8settle]` step between `[4i/8align]` pin writes and `[5/8] StartRecord`: wait on the genlock-fifo audit relock/underrun/dropped_due/late_hold DELTAS going quiet, NOT a blind sleep; raw counter-value deltas so #797-immune; TWO independent bounds — wall budget + pass ceiling — fail-open never abort; ALWAYS-exit-0 under the caller's `set -euo pipefail` (#1133 class); injectable reader/clock/sleep seams for Tier-0; issue 1221) → `.claude/rules/genlock-settle-wait.md` (auto-loads on its `paths:`)
