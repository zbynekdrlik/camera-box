...
- Why strih+stream OBS auto-RECORD on stream go-live and orphan after StopStream (the recurring RIG_BUSY-blocks-E2E cause): NOT camera-box, NOT `RecordWhenStreaming`, NOT adv-ss — it is a Bitfocus Companion controller (10.77.9.205) whose enabled `PRODUCTION` trigger (condition_true: stream scene "PRO" AND streaming) presses a go-live combo StartRecording BOTH boxes and has NO condition_false counterpart; the symmetric StopRecord button already exists (24/1/2) but nothing auto-presses it. camera-box's #657 stray-signature self-heal already bounds it (no code change); the fix is owner-side Companion. Full mechanism + exact config quotes + the recommended `PRODUCTION OFF` trigger + read-only re-inspect commands; #1274 → `.claude/rules/strih-autorecord-coupling.md` (auto-loads on its `paths:`)
