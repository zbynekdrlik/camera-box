...
- RESOLUME-SNV cg OBS on the genlock fleet (the `resolume` arm in deploy-genlock-fleet.sh / launch-obs-genlock.sh: win-resolume MCP, has_ahk=0, hostname-not-pinned-IP + live identity-confirm, explicit-only never the empty-default fleet; the genlock build DEFAULTS genlock_fifo=true + latency 3 + certified coercion so the "pin" step is a CONFIRM not a WS write; the `_ndi_inputs_matching` sp-*_video prefix-floor sentinel in latency_pins_verify.py/latency-pins-baseline.json; NDI output `RESOLUME-SNV (cg-obs)` + #1185 :5961 pin; the supervisor deploy→relaunch→confirm→cg-chain-verify runbook; #1295) → `.claude/rules/resolume-cg-obs.md` (auto-loads on its `paths:`)
