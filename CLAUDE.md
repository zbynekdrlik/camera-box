...
- strih-lx OBS projector Linux child-host (the XWayland+NVIDIA-PRIME present stall: a projector whose GL surface IS the X toplevel blocks the graphics thread ~0.5s/present, lag 93%→0% / MV 1.8→29.8fps when hosted in a child window; `#if __linux__` creation-site host toplevel + `Qt::Widget` child via `projectorWindowFlags`; the `Toplevel()==window()` accessor routes EVERY toplevel-only call incl. the runtime `SetAlwaysOnTop`/`UpdateProjectorAlwaysOnTop`/setWindowIcon ones — not just the ctor; host teardown both directions via eventFilter+`closing`+~dtor deleteLater; `isOBSProjectorWindow` is Windows-only so null-guard the toplevel handle; Windows byte-identical, FRONTEND=full-bundle deploy; #1352) → `.claude/rules/obs-projector-child-host.md` (auto-loads on its `paths:`)
