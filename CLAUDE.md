...
- Receiver-side AUDIO genlock parity (#1303 — the audio leg made a first-class genlocked signal: the audio HOLD delayed at `source_output_audio_data` by the same latency_ms the video FIFO holds video, orthogonal to the ASRC rate servo; the pure `src/genlock_audio_pairing.rs` decision + 3 static-inline C mirrors + parity gate; the audio facet in `obs_genlock_stats` v2 + the `genlock-fifo audit` line + jitter_audit; the cg-chain audio column; the 3-copy lock-step + the Tier-0 recipes; deferred followups = the LOCK-indicator audio term + the per-box certified-table audit) → `.claude/rules/genlock-audio-pairing.md` (auto-loads on its `paths:`)
