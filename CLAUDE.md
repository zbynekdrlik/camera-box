...
- Per-cambox HDMI-splitter-port no-signal recurrence watch (dev1-side timer reads each ACTIVE cambox's last `capture chroma:` journal line; the SELF-ANCHORING discriminator — a box pages a SPLITTER-PORT suspicion only when it is degraded AND ≥1 sibling on the SAME camera+splitter is proven-good, all-dark = SOURCE_WIDE report-only; the FIRST check for "weird colours on some cameras" = per-box signal presence not card tuning; Elgato purple-noise residual; #739) → `.claude/rules/splitter-port-health-watchdog.md` (auto-loads on its `paths:`)
