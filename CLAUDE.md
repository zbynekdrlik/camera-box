...
- dev1 dante-clock alert watchdog (#1307, PRODUCTION-CRITICAL class, umbrella #1308): headless-fleet clock-loss paging — reads EVERY dantesync node's `:8898/status` (cam1-7 via camera-set.sh `camera_resolve` incl. retired-but-powered cam5-7, + strih/stream/imag/resolume via obs-fleet.sh; resolume home-gated) and pages on not-locked / foreign-gm / `ntp_step_storm` (dantesync's OWN 120/h boolean, never a re-hardcoded literal) / DNS `video-clock.lan` unresolvable / grandmaster IP MOVED between passes; pure `dantesync_clock_decision.py` (#1199 mirror, grading mirrors clock-offset-guard.sh field semantics) + `dantesync-clock-alert-watchdog.sh` reusing obs-watchdog-decision.sh confirm + rig-grandmaster.sh; TIME-BUCKETED `--dedup-key dante-clock-<box>-<floor(now/REPING_INTERVAL_S)>` re-pings „dokolecka" while it persists (owner ruling — the deliberate exception to watchdog-notify-dedup.md, allowlisted in the #1206 sweep), recovery log-only; UNREACHABLE=SKIP defers #1001; ships DISABLED (supervisor enables on dev1) → `.claude/rules/dantesync-clock-alert-watchdog.md` (auto-loads on its `paths:`)
