...
use crate::ndi_video::pattern::test_pattern_rgb;
...
#[cfg(not(feature = "ndi"))]
pub fn build_default_source(source_name: &str) -> anyhow::Result<Box<dyn VideoSource>> {
...
pub fn build_default_source(source_name: &str) -> anyhow::Result<Box<dyn VideoSource>> {
    Ok(Box::new(
        crate::ndi_video::ndi_source::NdiVideoSource::connect(source_name)?,
    ))
}
