//! copied from bkshading/service/src/preview/ (bandwidth LOWEST, BGRX/BGRA colour) — keep in sync (issue 1345 M3c).
...
//! math is delegated to the pure, unit-tested [`crate::ndi_video::convert`] functions.
//!
...
use crate::ndi_video::convert::{bgra_to_rgb, rgba_to_rgb, uyvy_to_rgb};
use crate::ndi_video::frame::RawFrame;
use crate::ndi_video::ndi_paths::{current_ndi_os, ndi_search_candidates};
use crate::ndi_video::shared_runtime::SharedRuntime;
...
/// A low-bandwidth NDI receiver presented as a [`VideoSource`].
...
unsafe impl Send for NdiVideoSource {}
...
impl NdiVideoSource {
    ...
    pub fn connect(source_name: &str) -> Result<Self> {
        ...
        let recv_name = CString::new("intercom-video").context("recv name")?;
...
impl VideoSource for NdiVideoSource {
    fn name(&self) -> &str {
        &self.name
    }
